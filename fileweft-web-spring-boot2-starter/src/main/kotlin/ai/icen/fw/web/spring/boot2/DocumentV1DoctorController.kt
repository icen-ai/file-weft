...
/** Spring Boot 2 MVC edge for document-scoped formal Doctor operations. */
@RestController
@RequestMapping(value = ["/fileweft/v1/documents"], produces = [MediaType.APPLICATION_JSON_VALUE])
class DocumentV1DoctorController(
    ...
    ): ResponseEntity<ApiResponse<*>> = execute(HttpStatus.OK) {
    ...
    ): ResponseEntity<ApiResponse<*>> = execute(HttpStatus.ACCEPTED) {
    ...
    ): ResponseEntity<ApiResponse<*>> = methodNotAllowed("GET")

    @RequestMapping(value = ["/{documentId}/doctor/tasks"], method = [RequestMethod.HEAD])
    fun scheduleHead(
        @Suppress("UNUSED_PARAMETER") @PathVariable("documentId") documentId: String,
    ): ResponseEntity<ApiResponse<*>> = methodNotAllowed("POST")
    ...
    private fun execute(status: HttpStatus, action: () -> Any): ResponseEntity<ApiResponse<*>> {
    ...
    private fun methodNotAllowed(allowedMethod: String): ResponseEntity<ApiResponse<*>> {
        val mapped = responses.failure(V1MethodNotAllowedException(), currentTraceId())
        val response: ApiResponse<*> = mapped.response
        return response(HttpStatus.valueOf(mapped.status.statusCode), response, allowedMethod)
    }
