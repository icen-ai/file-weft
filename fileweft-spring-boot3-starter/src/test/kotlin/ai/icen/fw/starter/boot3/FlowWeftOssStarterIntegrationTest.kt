...
class FlowWeftOssStarterIntegrationTest {
    @Test
    fun `Boot 3 context composes the real OSS adapter for a host credential provider`() {
        ...
        ApplicationContextRunner()
            ...
            .withPropertyValues(
                ...
                "fileweft.default-tenant-id=oss-starter-boot3",
    ...
    private fun assertRoundTrip(adapter: StorageAdapter) {
        val content = "flowweft-boot3-oss-starter-${UUID.randomUUID()}".toByteArray(StandardCharsets.UTF_8)
        val stored = adapter.upload(
            StorageUploadRequest(
                tenantId = Identifier("oss-starter-boot3-${UUID.randomUUID()}"),
                objectName = "starter-context-smoke.txt",
                contentLength = content.size.toLong(),
                contentType = "text/plain",
                metadata = mapOf("suite" to "boot3-starter-context"),
            ),
