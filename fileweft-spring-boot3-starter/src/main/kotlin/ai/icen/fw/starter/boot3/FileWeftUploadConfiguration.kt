...
class FileWeftUploadConfiguration {
    ...
    @Bean(name = ["resumableUploadSessions"])
    @ConditionalOnMissingBean(ResumableUploadSessionRepository::class)
    fun jdbcResumableUploadSessions(objectMapper: ObjectMapper): JdbcResumableUploadSessionRepository =
    ...
    fun resumableUploadSessions(objectMapper: ObjectMapper): ResumableUploadSessionRepository =
        factories.resumableUploadSessions(objectMapper)
    ...
    fun presignedUploadMetricsPublisher(
    ...
    @ConditionalOnProperty(prefix = "fileweft.upload", name = ["presigned-enabled"], havingValue = "true")
    fun presignedUploadDoctorPlugin(diagnostics: PresignedUploadDiagnosticsService): FileWeftPlugin =
    ...
    fun completedPresignedUploadAssetClaimService(
        tenants: TenantProvider,
        users: UserRealmProvider,
        authorization: AuthorizationProvider,
    ...
    fun presignedUploadRecoveryService(
        sessions: PresignedUploadSessionRepository,
        clock: Clock,
        transaction: ApplicationTransaction,
        properties: FileWeftProperties,
    ...
    fun presignedUploadCleanupService(
        sessions: PresignedUploadSessionRepository,
        storageCandidates: ObjectProvider<StorageAdapter>,
        clock: Clock,
        transaction: ApplicationTransaction,
    ...
    fun completedUploadAssetClaimService(
        tenants: TenantProvider,
        users: UserRealmProvider,
        authorization: AuthorizationProvider,
    ...
    ) = CompletedResumableUploadAssetClaimService(
        tenantProvider = tenants,
        userRealmProvider = users,
        authorizationProvider = authorization,
        uploadSessions = sessions,
