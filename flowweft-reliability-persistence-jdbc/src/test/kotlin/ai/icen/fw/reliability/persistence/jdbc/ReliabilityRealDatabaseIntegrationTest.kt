...
import ai.icen.fw.reliability.persistence.migration.ReliabilityFlywayMigrationRunner
...
abstract class ReliabilityRealDatabaseIntegrationContract {
    ...
    @Test
    fun `V039 baseline then V040 migration is idempotent and owns the reliability namespace`() {
        ...
        val namespace = "fw_reliability_${java.lang.Long.toUnsignedString(System.nanoTime(), 36)}"
        createIsolatedDatabase(namespace).use { isolated ->
            val runner = ReliabilityFlywayMigrationRunner(isolated.dataSource, namespace)
            assertEquals(1, runner.migrate(), "first migrate should apply V040")
            runner.validate()
            ...
            isolated.dataSource.connection.use { connection ->
                ...
                val historyExists = connection.metaData.getTables(null, namespace, "%", null).use { result ->
                    generateSequence { if (result.next()) result.getString("TABLE_NAME") else null }
                        ...
                        .toList()
                        .contains(ReliabilityFlywayMigrationRunner.HISTORY_TABLE)
                }
                assertTrue(historyExists, "dedicated reliability history table must exist")
            }
...
class ReliabilityPostgresIntegrationTest : ReliabilityRealDatabaseIntegrationContract() {
...
class ReliabilityKingbaseIntegrationTest : ReliabilityRealDatabaseIntegrationContract() {
...
class ReliabilityMySQLIntegrationTest : ReliabilityRealDatabaseIntegrationContract() {
...
private class DriverManagerDataSource(
    ...
    override fun getParentLogger(): Logger = Logger.getLogger("flowweft.reliability.jdbc")
    override fun <T : Any?> unwrap(iface: Class<T>): T {
        if (iface.isInstance(this)) return iface.cast(this)
        throw SQLException("FlowWeft Reliability test DataSource does not wrap ${iface.name}.")
    }
