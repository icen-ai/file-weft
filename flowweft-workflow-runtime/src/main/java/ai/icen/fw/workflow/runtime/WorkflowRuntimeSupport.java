...
/** Package-private Java 8 support for bounded immutable runtime contracts. */
final class WorkflowRuntimeSupport {
    static final int MAX_CODE_BYTES = 128;
    static final int MAX_ID_BYTES = 512;
    static final int MAX_TEXT_BYTES = 1024;
    static final int MAX_ITEMS = 4096;

    private WorkflowRuntimeSupport() {
    }
    ...
    static String text(String value, int maximumBytes, String message) {
        ...
        while (offset < value.length()) {
            ...
            require(utf8Bytes <= maximumBytes, message);
        }
    ...
    static int positive(int value, int maximum, String message) {
        require(value > 0 && value <= maximum, message);
        return value;
    }

    static <T> List<T> immutable(Collection<? extends T> values, int maximum, String message) {
    ...
    static final class Digest {
        ...
        Digest(String domain) {
            code(domain, "Workflow runtime digest domain is invalid.");
            if (!domain.startsWith("flowweft-workflow-runtime-")) {
                throw new IllegalArgumentException("Workflow runtime digest domain is invalid.");
            }
            try {
        ...
        Digest text(String value) {
            Objects.requireNonNull(value, "Workflow runtime digest text is null.");
        ...
        String finish() {
            if (finished) throw new IllegalStateException("Workflow runtime digest is finalized.");
            finished = true;
