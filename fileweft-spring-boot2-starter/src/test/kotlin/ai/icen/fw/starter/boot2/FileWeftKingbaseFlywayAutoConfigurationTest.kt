...
class FileWeftKingbaseFlywayAutoConfigurationTest {
    private val contextRunner = ApplicationContextRunner()
        .withConfiguration(AutoConfigurations.of(FileWeftKingbaseFlywayAutoConfiguration::class.java))
    ...
    fun `wraps Flyway final Kingbase data source without replacing the main data source`() {
        ...
        contextRunner
            ...
            .run { context ->
                ...
                assertSame(mainDataSource, context.getBean(DataSource::class.java))
    ...
    @Test
    fun `property opt out does not register Flyway compatibility beans`() {
        val mainDataSource = MetadataDataSource.kingbase()
        ...
        contextRunner
            ...
            .run { context ->
                assertNull(context.startupFailure)
                assertFalse(context.containsBean(FILEWEFT_KINGBASE_FLYWAY_CUSTOMIZER_BEAN))
    ...
    @Test
    fun `adapts a host owned dedicated Flyway data source without replacing application data source`() {
        contextRunner
            .withUserConfiguration(HostFlywayDataSourceConfiguration::class.java)
            .run { context ->
                assertNull(context.startupFailure)
                assertTrue(context.containsBean(FILEWEFT_KINGBASE_FLYWAY_CUSTOMIZER_BEAN))
                assertTrue(context.containsBean("hostFlywayDataSource"))
                val applicationDataSource = context.getBean("dataSource", DataSource::class.java)
                val hostFlywayDataSource = context.getBean("hostFlywayDataSource", DataSource::class.java)
                val selectedDataSource = applyCustomizers(context, hostFlywayDataSource).dataSource

                assertSame(applicationDataSource, context.getBean(DataSource::class.java))
                assertNotSame(hostFlywayDataSource, selectedDataSource)
                selectedDataSource.connection.use { connection ->
                    assertEquals("PostgreSQL", connection.metaData.databaseProductName)
                    assertEquals(KINGBASE_URL, connection.metaData.url)
                }
                hostFlywayDataSource.connection.use { connection ->
                    assertEquals("KingbaseES", connection.metaData.databaseProductName)
                }
            }
    }
    ...
    @Test
    fun `adapts the sole host data source when it is already Flyway qualified`() {
        contextRunner
            .withUserConfiguration(HostSingleFlywayDataSourceConfiguration::class.java)
            .run { context ->
                assertNull(context.startupFailure)
                assertTrue(context.containsBean(FILEWEFT_KINGBASE_FLYWAY_CUSTOMIZER_BEAN))
                val hostDataSource = context.getBean("dataSource", DataSource::class.java)
                val selectedDataSource = applyCustomizers(context, hostDataSource).dataSource

                assertSame(hostDataSource, context.getBean(DataSource::class.java))
                assertNotSame(hostDataSource, selectedDataSource)
                selectedDataSource.connection.use { connection ->
                    assertEquals("PostgreSQL", connection.metaData.databaseProductName)
                    assertEquals(KINGBASE_URL, connection.metaData.url)
                }
                hostDataSource.connection.use { connection ->
                    assertEquals("KingbaseES", connection.metaData.databaseProductName)
                }
            }
    }
    ...
    @Test
    fun `does not guess a Flyway data source when the host has multiple application data sources`() {
        contextRunner
            .withBean("firstDataSource", DataSource::class.java, { MetadataDataSource.kingbase() })
            .withBean("secondDataSource", DataSource::class.java, { MetadataDataSource.postgres() })
            .run { context ->
                assertNull(context.startupFailure)
                assertTrue(context.containsBean(FILEWEFT_KINGBASE_FLYWAY_CUSTOMIZER_BEAN))
            }
    ...
    @Test
    fun `FileWeft migration runner keeps the original Kingbase data source`() {
        val mainDataSource = MetadataDataSource.kingbase()

        contextRunner
            .withUserConfiguration(FileWeftMigrationConfiguration::class.java)
            .withBean("dataSource", DataSource::class.java, { mainDataSource })
            .withBean(
                "hostMigrationInitializer",
                FileWeftMigrationInitializer::class.java,
                { NoOpMigrationInitializer },
            )
            .withPropertyValues(
            ...
            .run { context ->
                ...
                val runner = context.getBean(FlywayMigrationRunner::class.java)
                val runnerDataSource = FlywayMigrationRunner::class.java
                    .getDeclaredField("dataSource")
                    .apply { isAccessible = true }
                    .get(runner) as DataSource
                assertSame(mainDataSource, runnerDataSource)
                runnerDataSource.connection.use { connection ->
                    assertEquals(
                        FlywayMigrationRunner.DatabaseProduct.KINGBASE,
    ...
    private object NoOpMigrationInitializer : FileWeftMigrationInitializer {
        override fun initialize() = Unit
    }

    @Configuration(proxyBeanMethods = false)
    internal class HostFlywayDataSourceConfiguration {
        @Bean("dataSource")
        @Primary
        fun dataSource(): DataSource = MetadataDataSource.postgres()

        @Bean("hostFlywayDataSource")
        @FlywayDataSource
        fun hostFlywayDataSource(): DataSource = MetadataDataSource.kingbase()
    }

    @Configuration(proxyBeanMethods = false)
    internal class HostSingleFlywayDataSourceConfiguration {
        @Bean("dataSource")
        @FlywayDataSource
        fun dataSource(): DataSource = MetadataDataSource.kingbase()
    }
