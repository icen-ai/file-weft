...
class KingbaseFlywayAutoConfigurationIntegrationTest {
    @Test
    fun `Boot 2 Flyway auto configuration migrates an independent host location on Kingbase`() {
        ...
        try {
            ApplicationContextRunner()
                ...
                .withPropertyValues(
                    ...
                    "spring.flyway.locations=classpath:db/kingbase-host-boot2",
                ...
                .run { context ->
                    ...
                    mainDataSource.connection.use { connection ->
                        ...
                        assertEquals("boot2-flyway8", singleString(connection, "SELECT runtime_name FROM $HOST_SCHEMA.$PROBE_TABLE"))
                        assertEquals(
                            1,
                            singleInt(
                                connection,
    ...
    @Test
    fun `Boot 2 adapts the dedicated Kingbase data source created from spring flyway url`() {
        check(System.getenv("FILEWEFT_RUN_KINGBASE_TESTS") == "true") {
            "Kingbase integration tests must run only through the fail-closed Gradle task"
        }
        assertEquals("1.8", System.getProperty("java.specification.version"))
        ...
        val settings = KingbaseSettings.fromEnvironment()
        Class.forName(settings.driver)
        settings.rawConnection().use { connection -> resetSchema(connection, URL_HOST_SCHEMA) }
        val mainDataSource = DriverManagerDataSource(settings)
        ...
        try {
            ApplicationContextRunner()
                ...
                .withPropertyValues(
                    "spring.flyway.enabled=true",
                    "spring.flyway.url=${settings.url}",
                    "spring.flyway.user=${settings.user}",
                    "spring.flyway.password=${settings.password}",
                    "spring.flyway.driver-class-name=${settings.driver}",
                    "spring.flyway.locations=classpath:db/kingbase-host-boot2",
                    "spring.flyway.default-schema=$URL_HOST_SCHEMA",
                    "spring.flyway.schemas=$URL_HOST_SCHEMA",
                    "spring.flyway.table=$URL_HISTORY_TABLE",
                    "spring.flyway.clean-disabled=true",
                )
                .run { context ->
                    assertNull(context.startupFailure)
                    ...
                    val flywayDataSource = context.getBean(Flyway::class.java).configuration.dataSource
                    assertNotSame(mainDataSource, flywayDataSource)
                    flywayDataSource.connection.use { connection ->
                        assertEquals("PostgreSQL", connection.metaData.databaseProductName)
                        assertEquals(settings.url, connection.metaData.url)
                    ...
                    mainDataSource.connection.use { connection ->
                        ...
                        assertEquals(
                            "boot2-flyway8",
                            singleString(connection, "SELECT runtime_name FROM $URL_HOST_SCHEMA.$PROBE_TABLE"),
                        )
                        assertEquals(
                            1,
                            singleInt(
                                connection,
                                "SELECT COUNT(*) FROM $URL_HOST_SCHEMA.$URL_HISTORY_TABLE " +
            ...
            settings.rawConnection().use { connection -> dropSchema(connection, URL_HOST_SCHEMA) }
        }
    }

    private fun resetSchema(connection: Connection, schema: String = HOST_SCHEMA) {
    ...
    private fun dropSchema(connection: Connection, schema: String = HOST_SCHEMA) {
        connection.createStatement().use { statement ->
            statement.execute("DROP SCHEMA IF EXISTS $schema CASCADE")
        }
    }
    ...
    private class DriverManagerDataSource(
        ...
        override fun getParentLogger(): Logger = Logger.getLogger("ai.icen.fw.starter.boot2.kingbase")
    ...
    private companion object {
        const val HOST_SCHEMA = "fw_boot2_host_probe"
        const val HISTORY_TABLE = "fw_boot2_host_schema_history"
        const val URL_HOST_SCHEMA = "fw_boot2_url_host_probe"
        const val URL_HISTORY_TABLE = "fw_boot2_url_schema_history"
        const val PROBE_TABLE = "fw_boot2_host_migration_probe"
    }
}
