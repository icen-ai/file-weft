...
## 历史产品决策：Agent 延后（已被 1.0 ADR 取代）

本文件是面向 `0.0.2` 文档重写的历史上下文。`0.0.2` 和 `0.0.3` 不提供 Agent
产品能力仍是历史事实；但“只能在 1.0 发布后重新评估”的未来开发限制已被
`docs/decisions/0001-flowweft-1.0-product-scope.md` 取代。当前开发必须以根
`AGENTS.md`、该 ADR 和 `docs/flowweft-1.0-delivery-ledger.md` 为准。

现有 `fileweft-agent`、Agent SPI/公共 ABI 及 V012/V026 中相关表和列仍仅为源码、
二进制与数据库兼容保留，不得改义承载新版 Agent。本文后续 Agent 内容只能用来
解释历史 ABI，不能覆盖新的 1.0 增量架构。
