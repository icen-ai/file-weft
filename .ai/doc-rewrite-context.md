...
正式 Maven group：`ai.icen`；JVM 包名：`ai.icen.fw`。当前稳定版 `ai.icen:*:0.0.1`。已撤回旧试推坐标 `com.fileweft:*:0.0.1`，不得使用。
...
- persistence：PostgreSQL、Flyway 迁移、仓储实现。
...
- 不要承诺尚未发布的 0.0.2-SNAPSHOT 能力为稳定版。
