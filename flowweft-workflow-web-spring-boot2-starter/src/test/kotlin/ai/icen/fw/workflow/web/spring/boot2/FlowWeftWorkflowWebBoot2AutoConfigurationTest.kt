...
class FlowWeftWorkflowWebBoot2AutoConfigurationTest {
    @Test
    fun `does not expose routes without a trusted context provider`() {
        runner().withUserConfiguration(MapperOnlyConfiguration::class.java).run { context ->
            assertTrue(context.getBeansOfType(FlowWeftWorkflowWebBoot2Controller::class.java).isEmpty())
    ...
    fun `registers transport with one trusted provider and backs off host codec`() {
        runner().withUserConfiguration(SecureHostConfiguration::class.java).run { context ->
            ...
            assertEquals(1, context.getBeansOfType(FlowWeftWorkflowWebBoot2ApplicationPorts::class.java).size)
            assertEquals(1, context.getBeansOfType(FlowWeftWorkflowWebBoot2Controller::class.java).size)
            assertSame(
                context.getBean("customerWorkflowJsonCodec"),
                context.getBean(FlowWeftWorkflowWebBoot2JsonCodec::class.java),
            )
        }
    }
    ...
    fun `registers through Boot 2 spring factories`() {
        ...
        assertTrue(factories.contains(FlowWeftWorkflowWebBoot2AutoConfiguration::class.java.name))
    }

    private fun runner(): WebApplicationContextRunner = WebApplicationContextRunner()
        .withConfiguration(AutoConfigurations.of(FlowWeftWorkflowWebBoot2AutoConfiguration::class.java))
    ...
    internal class SecureHostConfiguration {
        ...
        @Bean fun customerWorkflowJsonCodec(mapper: ObjectMapper): FlowWeftWorkflowWebBoot2JsonCodec =
            FlowWeftWorkflowWebBoot2JsonCodec(mapper)
    }
