...
/** Fail-closed MySQL boundary: V030 relies on enforced CHECK constraints from 8.0.17+. */
internal object AgentMySqlDatabaseSupport {
    fun requireSupported(metadata: DatabaseMetaData) {
        ...
        val match = VERSION_PREFIX.find(version)
            ?: error("Cannot verify MySQL version '$version'; FlowWeft Agent requires MySQL 8.0.17+ in the 8.x line.")
        val (major, minor, patch) = match.destructured.toList().map(String::toInt)
        check(major == 8 && (minor > 0 || patch >= 17)) {
            "Unsupported MySQL version '$version'; FlowWeft Agent requires MySQL 8.0.17+ in the 8.x line."
        }
    }
