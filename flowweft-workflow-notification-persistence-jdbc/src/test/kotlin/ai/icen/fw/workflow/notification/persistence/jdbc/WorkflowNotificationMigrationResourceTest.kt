...
class WorkflowNotificationMigrationResourceTest {
    @Test
    fun `V035 has three dialect parity and durable delivery boundaries`() {
        val migrations = listOf("postgres", "mysql", "kingbase").associateWith { dialect ->
            val path = "/ai/icen/fw/workflow/notification/db/migration/$dialect/" +
                "V035__persist_workflow_notification_lifecycle.sql"
        ...
        migrations.forEach { (dialect, sql) ->
            ...
            listOf("authorization_header", "bearer_token", "cookie_value", "provider_secret", "endpoint_url")
                .forEach { forbidden ->
                    assertFalse(lower.contains(forbidden), "$dialect V035 persists forbidden $forbidden")
                }
        }
    }

    @Test
    fun `migration locations are explicit and never auto execute`() {
        ...
        assertEquals(
            ...
            WorkflowNotificationJdbcMigrations.location(WorkflowNotificationJdbcDialect.POSTGRESQL),
        )
        assertEquals(
        ...
        assertEquals(
            ...
            WorkflowNotificationJdbcMigrations.location(WorkflowNotificationJdbcDialect.KINGBASE),
        )
    }
