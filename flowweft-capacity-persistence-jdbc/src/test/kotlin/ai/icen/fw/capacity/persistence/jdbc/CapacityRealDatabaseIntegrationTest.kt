...
import ai.icen.fw.capacity.persistence.migration.CapacityFlywayMigrationRunner
...
abstract class CapacityRealDatabaseIntegrationContract {
    ...
    @Test
    fun `V038 baseline then V039 migration is idempotent and owns the capacity namespace`() {
        ...
        val namespace = "fw_capacity_${java.lang.Long.toUnsignedString(System.nanoTime(), 36)}"
        createIsolatedDatabase(namespace).use { isolated ->
            val runner = CapacityFlywayMigrationRunner(isolated.dataSource, namespace)
            assertEquals(1, runner.migrate(), "first migrate should apply V039")
            runner.validate()
            ...
            isolated.dataSource.connection.use { connection ->
                ...
                val historyExists = connection.metaData.getTables(null, namespace, "%", null).use { result ->
                    generateSequence { if (result.next()) result.getString("TABLE_NAME") else null }
                        ...
                        .toList()
                        .contains(CapacityFlywayMigrationRunner.HISTORY_TABLE)
                }
                assertTrue(historyExists, "dedicated capacity history table must exist")
            }
...
class CapacityPostgresIntegrationTest : CapacityRealDatabaseIntegrationContract() {
...
class CapacityKingbaseIntegrationTest : CapacityRealDatabaseIntegrationContract() {
...
class CapacityMySQLIntegrationTest : CapacityRealDatabaseIntegrationContract() {
...
private class DriverManagerDataSource(
    ...
    override fun getParentLogger(): Logger = Logger.getLogger("flowweft.capacity.jdbc")
    override fun <T : Any?> unwrap(iface: Class<T>): T {
        if (iface.isInstance(this)) return iface.cast(this)
        throw SQLException("FlowWeft Capacity test DataSource does not wrap ${iface.name}.")
    }
