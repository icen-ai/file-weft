package ai.icen.fw.capacity.persistence.migration

import ai.icen.fw.capacity.persistence.jdbc.CapacityMySqlDatabaseSupport
...
/** Capacity-only V039+ migration boundary with an isolated history table and resource location. */
class CapacityFlywayMigrationRunner @JvmOverloads constructor(
    ...
    fun validate() {
        ...
        check(historyExists(target)) {
            "Capacity migration validation requires ${target.schema}.$HISTORY_TABLE; run migrate first."
        }
        val flyway = configuredFlyway(target)
        flyway.validate()
        check(flyway.info().pending().isEmpty()) { "Capacity migration validation found pending V039+ migrations." }
        verifyNamespace(target)
    }

    private fun configuredFlyway(target: DatabaseTarget): Flyway {
        val flywayDataSource = if (target.product == DatabaseProduct.KINGBASE) {
            CapacityKingbaseFlywayDataSource(dataSource)
        } else {
            dataSource
        }
        return Flyway.configure()
            ...
            .baselineDescription("FlowWeft Capacity V039 namespace")
    ...
    private fun resolveTarget(): DatabaseTarget = dataSource.connection.use { connection ->
            ...
            val current = connection.createStatement().use { statement ->
                statement.executeQuery(sql).use { result ->
                    check(result.next()) { "Capacity DataSource did not expose a current schema." }
                    result.getString(1)?.let(::safeSchema)
                }
            }
            val actual = checkNotNull(current) { "Capacity migration requires a non-null current schema." }
            check(configuredSchema == null || configuredSchema == actual) {
                "Configured Capacity schema '$configuredSchema' does not equal DataSource current schema '$actual'."
            }
    ...
    private fun requireNoUntrackedOwnedTables(target: DatabaseTarget) {
        ...
        check(present.isEmpty() || historyExists(target)) {
            "Capacity migration history is absent but managed tables already exist: ${present.sorted().joinToString()}."
        }
    }
    ...
    private fun verifyNamespace(target: DatabaseTarget) {
        dataSource.connection.use { connection ->
            ...
            check(missing.isEmpty()) {
                "Capacity migration namespace is incomplete; missing ${missing.sorted().joinToString()}."
            }
            val rows = historyRows(connection, target)
            check(rows.any { row -> row.version == BASELINE_VERSION && row.type == "BASELINE" && row.success }) {
                "Capacity migration history lacks its V038 baseline sentinel."
            }
            rows.forEach { row ->
                check(row.success) { "Capacity migration history contains an unsuccessful row." }
                val numeric = row.version?.toIntOrNull()
                check(numeric == null || numeric >= FIRST_CAPACITY_VERSION ||
                    numeric == BASELINE_VERSION.toInt() && row.type == "BASELINE"
                ) {
                    "Capacity migration history contains a pre-V039 non-baseline migration."
                }
            }
        }
    }
    ...
    private fun safeSchema(value: String): String {
        require(SAFE_SCHEMA.matches(value)) { "Capacity migration schema must be a simple non-empty SQL identifier." }
        return value
    }
    ...
    companion object {
        const val HISTORY_TABLE: String = "flowweft_capacity_schema_history"
        const val BASELINE_VERSION: String = "38"
        const val FIRST_CAPACITY_VERSION: Int = 39
        ...
        fun detectProduct(connection: Connection): DatabaseProduct {
            ...
            return when {
                ...
                product.equals("MySQL", ignoreCase = true) -> {
                    CapacityMySqlDatabaseSupport.requireSupported(connection.metaData)
                    DatabaseProduct.MYSQL
                }
                product.startsWith("Kingbase", ignoreCase = true) -> DatabaseProduct.KINGBASE
                else -> throw IllegalStateException(
                    "Unsupported Capacity database '$product'; expected PostgreSQL, MySQL 8 or KingbaseES.",
                )
            }
        }
