...
const zh: ConsoleMessages = {
  ...
  pages: {
    ...
    approvals: {
      eyebrow: "02 / 通用流程工作台",
      title: "让任务、实例与协作证据沿同一条时间线展开。",
      summary: "只展示可信身份可读的定义、待办、实例、历史、表单摘要和评论；每条对象关系都在服务端投影后再次校验。",
      deliverable: "FW10-027 / 033 / 036 / 040 / 043",
      cards: [
        { title: "当前身份待办", detail: "服务端过滤后的通用人类任务与允许动作。", boundary: "浏览器不能传入 assignee 或 tenant filter。" },
        { title: "实例证据", detail: "任务关联实例、版本、主体、历史和安全评论。", boundary: "不存在与无权限使用同一失败投影。" },
        { title: "定义账本", detail: "版本、codec、摘要与发布诊断。", boundary: "定义源码和可执行结构不进入浏览器。" },
      ],
      emptyTitle: "Workflow 能力当前不可用",
      emptyDetail: "页面不会用旧审批接口或样例任务降级；mutation 会等 CSRF、If-Match 与幂等契约完整后再开放。",
      proofs: ["对象归属与跨租户拒绝", "历史/评论游标和安全 @ 提及", "中英文读屏与键盘操作"],
    },
    sync: {
      eyebrow: "03 / 同步中心",
...
const en: ConsoleMessages = {
  ...
  pages: {
    ...
    approvals: {
      eyebrow: "02 / GENERIC WORKFLOW WORKBENCH",
      title: "Unfold tasks, instances and collaboration evidence on one timeline.",
      summary: "Only definitions, tasks, instances, history, form metadata and comments readable by the trusted identity are shown, with server-projected object relationships verified again.",
      deliverable: "FW10-027 / 033 / 036 / 040 / 043",
      cards: [
        { title: "Current-principal tasks", detail: "Server-filtered generic human tasks and allowed actions.", boundary: "The browser cannot supply assignee or tenant filters." },
        { title: "Instance evidence", detail: "Related instance, version, subject, history and safe comments.", boundary: "Missing and forbidden objects share one failure projection." },
        { title: "Definition ledger", detail: "Versions, codec identity, digests and publication diagnostics.", boundary: "Definition source and executable structure stay out of the browser." },
      ],
      emptyTitle: "Workflow capability is unavailable",
      emptyDetail: "The page never falls back to legacy approval routes or sample tasks. Mutations wait for complete CSRF, If-Match and idempotency contracts.",
      proofs: ["Object ownership and cross-tenant rejection", "History/comment cursors and safe mentions", "Bilingual screen-reader and keyboard checks"],
    },
    sync: {
