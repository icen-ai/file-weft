...
这是 FW10-027/FW10-033/FW10-036/FW10-040/FW10-041/FW10-043/FW10-044 的增量实现，认证地基和部分只读业务接线可用，
但还不是完成全部 mutation 与浏览器验收的产品：
...
- 已有实时通用 Workflow 只读工作台：通过服务端会话固定调用
  `/flowweft/v1/workflows` 的定义、任务、实例、历史、评论与表单读取路由，严格校验 v1
  envelope、opaque cursor、任务/实例/主体/评论归属；定义源码、表单 schema/投影数据、
  tenant、其他参与者 principal id 与上游异常不会进入浏览器 DTO。定义或协作子能力可独立
  失败关闭，不会回退到历史 `/fileweft/v1/workflows/tasks` 或模拟数据；
- Workflow mutation 暂不开放：领取、决定、转办、加签、退回、表单提交与评论创建必须等
  Console 同源 CSRF、`If-Match`、`Idempotency-Key`、结果未知协调和审计链全部接线；
- 已有实时 Agent 只读工作台：通过服务端会话固定调用 `/flowweft/v1/agent` 的对话、运行、
  消息、事件与引用路由，严格校验 Web v1 envelope/游标/对象绑定，只向浏览器投影用户与助手
  可见文本、预算用量、耐久状态和二次授权后的引用元数据；隐藏对象与上游不可用不会回显
  请求标识、tenant、endpoint、Provider 载荷或异常正文；
...
| `/{zh,en}/approvals` | 当前会话实时、只读的通用 Workflow 定义、待办、实例、历史、评论与表单摘要工作台；子能力独立失败关闭 |
| `/{zh,en}/sync` | 同步状态页 |
| `/{zh,en}/doctor` | 当前租户实时、脱敏的系统 Doctor；不可用时安全降级为状态页 |
| `/{zh,en}/audit` | 审计状态页 |
| `/{zh,en}/agent` | 当前会话的实时只读 Agent 对话、运行、消息、事件与授权引用工作台；能力缺失时失败关闭 |
...
`server-only` 边界，以及 Agent Web 固定路由、严格 envelope、对象/游标绑定、过期引用
失败关闭与最小无 tenant 投影。
...
5. 在已接通的 Agent 只读工作台上，增加具有独立同源 Route Handler、CSRF、新鲜授权、原始
   `Idempotency-Key` 服务端处理和精确 `If-Match` 的创建对话、发起运行与取消 mutation；在这些
   条件完整前不渲染伪可用按钮，也不允许浏览器直连 Agent Web API；随后接工具确认与评测；
6. 增加 SSE/游标断线恢复、真实 IdP + Playwright、键盘/读屏、双语、session/source/tenant
   隔离和浏览器安全头验收。
