...
class JdbcAuditRecordRepository(
    ...
    override fun append(record: AuditRecord) {
        JdbcConnectionContext.requireCurrent().prepareStatement(
            "INSERT INTO fw_audit_record(id, tenant_id, resource_type, resource_id, action, operator_id, detail_json, created_time, updated_time) VALUES (?, ?, ?, ?, ?, ?, ?::jsonb, ?, ?)",
        ).use { statement ->
            ...
            statement.setString(7, objectMapper.writeValueAsString(record.details))
    ...
    override fun findByResource(tenantId: Identifier, resourceType: String, resourceId: Identifier, limit: Int): List<AuditRecord> {
        ...
        return JdbcConnectionContext.requireCurrent().prepareStatement(
            "SELECT id, tenant_id, resource_type, resource_id, action, operator_id, detail_json, created_time FROM fw_audit_record WHERE tenant_id = ? AND resource_type = ? AND resource_id = ? ORDER BY created_time DESC, id DESC LIMIT ?",
        ).use { statement ->
