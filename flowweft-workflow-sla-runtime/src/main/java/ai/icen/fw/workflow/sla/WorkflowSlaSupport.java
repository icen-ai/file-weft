...
/** Package-private Java 8 support for bounded, immutable SLA contracts. */
final class WorkflowSlaSupport {
    ...
    static final class Digest {
        ...
        Digest(String domain) {
            code(domain, "Workflow SLA digest domain is invalid.");
            if (!domain.startsWith("flowweft-workflow-sla-")) {
                throw new IllegalArgumentException("Workflow SLA digest domain is invalid.");
            }
            try {
        ...
        Digest text(String value) {
            Objects.requireNonNull(value, "Workflow SLA digest text is null.");
        ...
        String finish() {
            if (finished) throw new IllegalStateException("Workflow SLA digest is finalized.");
            finished = true;
