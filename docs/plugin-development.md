...
无目录宿主应暴露 `IdempotentDocumentLifecycleService`，启用目录的宿主只能暴露 `IdempotentDocumentCatalogLifecycleService`；后者会在每次重放前重新执行源目录 ACL，并在最终事务内复核绑定。不要在目录宿主同时把 flat 边界注册为候选，也不要从 Controller 直接调用 `reviseInCurrentTransaction` 等 application-internal ambient hook：这些 hook 只供幂等协调器在已经 claim 的同一最终事务中复用，调用方负责先完成授权、目录快照和事务外策略解析。
