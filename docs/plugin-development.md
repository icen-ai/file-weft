...
五套 TestKit 按产品边界独立发布：`fileweft-testkit` 保留存储、连接器、目录、Metadata、插件与可观测性合约；Agent Provider 使用 `ai.icen:flowweft-agent-testkit`；检索 Provider 使用 `ai.icen:flowweft-retrieval-testkit`；可靠性与治理 Provider 分别使用 `ai.icen:flowweft-reliability-testkit` 和 `ai.icen:flowweft-governance-testkit`。1.0 TestKit 都提供 Java 8 友好的入口，并用可继承 JUnit 契约验证自身夹具。独立的 `release-smoke/library-consumer` 必须只通过发布后的 Maven 坐标取类并让 JUnit 实际发现、执行代表性合约；它不是只编译或只实例化的仓库内夹具，也不会从旧 `fileweft-testkit` 偶然取得新 Provider API。真实 Provider 发布验收仍须在独立样例宿主中使用专用测试数据继承相同抽象类。异步等待默认有界，远程服务可覆写相应 timeout，但不得关闭结果绑定、授权 Gate、租户隔离、CAS/围栏或取消能力一致性断言。
