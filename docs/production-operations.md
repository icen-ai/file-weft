...
> **Agent 兼容边界：** `0.0.2` 与 `0.0.3` 均不提供 Agent 产品能力。默认运行时、
...
默认 Starter 会把宿主声明的普通 `MetadataSchema` Bean 作为当前版本，并把 `HistoricalMetadataSchema` Bean 包装的定义作为精确历史版本，随后构造进程内不可变 `MetadataSchemaRegistry`；同一 schema ID 只能有一个当前版本，重复 ID、重复 `id/version`、没有对应当前版本的历史项或非法/保留字段规则都会在启动时失败。该默认注册表不把 schema 定义写入 FileWeft 数据库，也不从 HTTP 动态创建 schema。多租户需要不同规则时，宿主应提供唯一的 `MetadataSchemaResolver` 替换默认注册表，并只依据 `MetadataSchemaContext` 中来自可信 `TenantProvider` 的 tenant、resource type 与 operation 路由，不能信任请求参数中的租户。
...
schema-qualified 创建与新增版本把业务 metadata 权威持久化在文档 `FileAsset.metadata` 中；新增版本执行完整的 schema metadata 替换，同时保留 `catalog.*` / `fileweft.*` 宿主框架键。该新路径不会把最多 64 KiB 的业务 metadata 复制进 S3 兼容对象的 user-metadata header，因为厂商 header 容量与读回能力不能充当 FileWeft 权威存储；未携带 schema metadata 的旧写入重载仍保持原有行为。
...
`V029` 向 `fw_workflow_instance` 加入可空的 `submitted_by varchar(256)`。历史审批没有可靠、不可歧义的提交者证据，因此迁移不回填；`NULL` 是有意保留的 UNKNOWN，不是待修复的空值。PostgreSQL 新增并验证与可信宿主用户 ID 一致的数据库约束；MySQL 与 KingbaseES 延续其他宿主用户列的兼容策略，在 FileWeft 可信应用边界执行完整 UTF-16 长度与不可见字符校验。新版本在创建工作流时保存当前已认证用户，后续撤回以该快照判断提交者身份。
