...
> **版本边界：** `0.0.2` 与 `0.0.3` 均不提供 Agent 产品能力。默认运行时、
> Doctor/插件清单、正式 HTTP 与 Dev 均不注册或暴露 Agent。现有
> `fileweft-agent`、Agent SPI/公共 ABI 及 V012/V026 数据库形状仅为兼容
> 保留。1.0 新架构见 [ADR 0001](decisions/0001-flowweft-1.0-product-scope.md)；
> 在其实现和验收完成前，本文后续 Agent 运行说明仍只适用于显式旧版兼容模式，
> 不是 1.0 部署建议。
...
## FlowWeft 迁移命名空间与旧库升级

FlowWeft 兼容迁移资源只发布在 `classpath:ai/icen/fw/db/migration`，并使用专属 `fileweft_schema_history`。它不会向宿主常用的 `classpath:db/migration` 放置资源，也不能与宿主 `flyway_schema_history` 合并；这样宿主和 FlowWeft 都可以拥有自己的 `V001`，而不会共享版本、checksum 或 repair 操作。
...
- `disabled` 表示 FlowWeft 完全不管理或校验数据库。只有外部发布系统已经以相同专属 location/history 完成迁移，或宿主有等价且经过评审的数据库发布流程时才应使用。
...
## Metadata schema 注册与版本保留

默认 Starter 会把宿主声明的普通 `MetadataSchema` Bean 作为当前版本，并把 `HistoricalMetadataSchema` Bean 包装的定义作为精确历史版本，随后构造进程内不可变 `MetadataSchemaRegistry`；同一 schema ID 只能有一个当前版本，重复 ID、重复 `id/version`、没有对应当前版本的历史项或非法/保留字段规则都会在启动时失败。该默认注册表不把 schema 定义写入 FlowWeft 数据库，也不从 HTTP 动态创建 schema。多租户需要不同规则时，宿主应提供唯一的 `MetadataSchemaResolver` 替换默认注册表，并只依据 `MetadataSchemaContext` 中来自可信 `TenantProvider` 的 tenant、resource type 与 operation 路由，不能信任请求参数中的租户。

文档资产会保存规范化 metadata 以及 `metadata.schema-id` / `metadata.schema-version`。发布新规则时应创建新 version，并继续以 `HistoricalMetadataSchema` 保留所有仍被历史资产引用的精确版本；不能原地改变同一 `id/version` 的语义。也可以显式提供 `MetadataSchemaRegistry(currentSchemas, historicalSchemas)` 或等价 resolver 完全接管。删除历史版本前必须先通过 Metadata Doctor 和数据盘点证明没有引用；缺失或不兼容时保持失败关闭，不得跳过校验或把原始 Map 当作已验证数据。

schema-qualified 创建与新增版本把业务 metadata 权威持久化在文档 `FileAsset.metadata` 中；新增版本执行完整的 schema metadata 替换，同时保留 `catalog.*` / `fileweft.*` 宿主框架键。该新路径不会把最多 64 KiB 的业务 metadata 复制进 S3 兼容对象的 user-metadata header，因为厂商 header 容量与读回能力不能充当 FlowWeft 权威存储；未携带 schema metadata 的旧写入重载仍保持原有行为。

MySQL 的 schema 就是 database，且无 database 的 JDBC URL 在 `CREATE DATABASE` 后建立的新连接仍不会自动选择该 database。为避免出现“DDL 已执行但应用仍失败”的半完成状态，FlowWeft 不从这种 DataSource 自动建 MySQL database：先由 DBA 预建 database，在 JDBC URL 中明确选择它，并保持 `create-schema=false`。若当前 `SELECT DATABASE()` 为 `null`，即使配置 `create-schema=true`，runner 也会在任何 DDL 前失败关闭。
...
#### KingbaseES 0.0.3 生产接入操作手册

以下 `0.0.3` 坐标只有在受保护 `v0.0.3` 标签流水线成功、19 个坐标完成匿名冷缓存回读后才可用于生产；本文和本地构建不证明远端已经可解析。消费 CNB 公共 Maven 仓库不需要把 CNB token 写入构建。Kingbase 驱动来自 Maven Central 或企业批准的受控镜像。Spring Boot 2 与 3 只能选择对应的一条 Starter 线：

Spring Boot 2 宿主还必须按[安装文档](../flowweft-docs/pages/zh/getting-started/installation.md)把 Kotlin 对齐到 `2.1.21`；其 BOM 默认的 `1.6.21` 不是 FlowWeft 运行时合同。
...
dependencies {
    ...
    implementation("ai.icen:fileweft-spring-boot2-starter:0.0.3")
    // implementation("ai.icen:fileweft-web-spring-boot2-starter:0.0.3") // 需要正式 HTTP API 时启用
...
dependencies {
    ...
    // implementation("ai.icen:fileweft-web-spring-boot3-starter:0.0.3") // 需要正式 HTTP API 时启用
    runtimeOnly("cn.com.kingbase:kingbase8:8.6.1")
}
```

DBA 必须先预建目标 schema；生产示例固定为兼容名称 `fileweft`，并保持 `create-schema=false`。官方锁定驱动的 `currentSchema` URL 属性会设置连接 search path，因此 URL、账号与 FlowWeft schema 的最小合同如下。Spring Boot 2：
...
这里的 `fileweft.persistence.schema` 是严格断言，不是切换开关：迁移账号和运行账号经该 URL 取出的每条连接执行 `SELECT current_schema()` 都必须精确返回 `fileweft`。如果企业连接池不用 URL 属性，必须提供在**每条连接**上执行 `SET search_path TO fileweft` 的等价初始化，并再次验证 `current_schema()`；不能只在部署脚本的单个会话中执行一次。多 DataSource 宿主仍须显式把 FlowWeft runner 绑定到这一 DataSource。
...
5. 若宿主自己使用 Spring Flyway，保留独立的宿主 location/history，不得追加 FlowWeft location；Boot 2 的 `spring.flyway.locations` 不得使用 `{vendor}`。宿主不使用 Flyway 时才设置 `spring.flyway.enabled=false`。
...
- [ ] 迁移账号只在发布窗口下发；运行账号只拥有业务 DML、schema `USAGE`、FlowWeft 业务读取及 migration history 校验所需权限。
- [ ] `KINGBASE_USERNAME` 与 `KINGBASE_PASSWORD` 由部署 Secret/密钥系统注入；禁止把密码写入 `application*.yml`、Gradle 文件、Git、镜像或日志。
- [ ] 宿主 Flyway 与 FlowWeft location/history 分离，多个 DataSource 的 runner 绑定明确。
- [ ] 迁移前完成可恢复备份，迁移失败时保留 history/日志证据并前进修复，禁止无条件 `repair` 或伪造成功记录。

KingbaseES 数据库镜像不得由 FlowWeft 仓库、CNB 制品库或其他镜像仓库再次分发。开发机与 CI 必须使用 `.ci/scripts/prepare-kingbase-image.ps1` / `.sh` 从金仓官网公开地址下载 V8R6C9B14 tar，并依次核对官网 MD5、仓库锁定 SHA-256 和导入后的 Docker image ID；任何校验失败都必须终止。不得用来历不明的社区镜像替代。
...
推荐由宿主提供的一次性 Migration Job 或受控迁移进程使用 `migrate`，成功后由宿主明确退出该进程，再让全部 Web/Worker 节点以 `validate` 启动；FlowWeft Starter 只负责启动时迁移，不提供迁移后自动退出的通用 Job 入口。所有角色必须指向相同 FlowWeft 兼容 schema；不要让每个滚动节点自行改变 migration mode，也不要把 FlowWeft 资源路径追加到宿主 `spring.flyway.locations`。Spring Boot 自己的 Flyway 可以继续管理宿主 schema 和默认 history，它与 `fileweft.persistence.*` 是两套独立边界。

权限应按角色拆分：`validate` 账号至少需要目标 schema 的 `USAGE`、FlowWeft 业务读取权限，以及 `fileweft_schema_history` 的只读权限；若目标 schema 也存在宿主 `flyway_schema_history`，旧记录检测还需要读取其中的 `script` 等迁移记录。Migration Job 的账号除上述读取权限外，还需要创建/修改 FlowWeft 表、索引和约束的 DDL 权限；`create-schema=true` 时才额外授予建 schema 权限。不要把 Migration Job 的 DDL 凭据长期下发给 Web/Worker。
...
FlowWeft 的 Web 节点默认不消费 Outbox 或后台任务。部署时推荐使用相同的应用工件启动两个角色：
...
若需要拆分资源池，可让下游同步节点只开启 `process-outbox`，让 Doctor 节点只开启 `process-tasks`，让存储维护节点只开启 `process-upload-cleanup`。所有节点可以水平扩展：Outbox 与后台任务均通过数据库租约/锁领取，重复投递由事件或任务的幂等键约束。不要为 `0.0.3` 默认部署规划 Agent 节点。
...
`0.0.3` 增加待审撤回：`POST /fileweft/v1/workflows/{workflowId}/withdraw` 只把仍为 `PENDING` 的工作流改为 `WITHDRAWN`，并把对应 `PENDING_REVIEW` 文档恢复为 `DRAFT`。新提交记录的可信 `submitted_by` 与当前用户一致时，提交者无需额外策略授权即可撤回，但目录宿主仍会重新检查当前可见性；其他操作者必须通过 `document:review:withdraw`。历史 `submitted_by IS NULL` 不代表任何用户，禁止从 task assignee、审计或幂等记录猜测，只能由策略授权的操作者处理。命令要求持久化幂等键并写审计；撤回、最后一票 approve/reject 和目录绑定竞态都遵守 idempotency → document → asset → workflow 锁序，因此只能有一个串行结果。
...
`V029` 向 `fw_workflow_instance` 加入可空的 `submitted_by varchar(256)`。历史审批没有可靠、不可歧义的提交者证据，因此迁移不回填；`NULL` 是有意保留的 UNKNOWN，不是待修复的空值。PostgreSQL 新增并验证与可信宿主用户 ID 一致的数据库约束；MySQL 与 KingbaseES 延续其他宿主用户列的兼容策略，在 FlowWeft 可信应用边界执行完整 UTF-16 长度与不可见字符校验。新版本在创建工作流时保存当前已认证用户，后续撤回以该快照判断提交者身份。

V029 与新撤回状态不能作为无停机混跑升级：旧节点不会写 `submitted_by`，也不了解 `WITHDRAWN`。维护窗口中先关闭 submit/approve/reject/withdraw 入口，停止全部旧 API 节点并等待在途审批事务结束；执行 V029、确认三种方言对应环境的 `fileweft_schema_history` 和列/约束，再一次性启动 `0.0.3` 节点。上线后至少验收一笔“提交者撤回”和一笔“策略操作者撤回”，同时核对文档、工作流、审计和同 key 重放结果。紧急应用回滚必须保留 V029 列及已经写入的身份证据，不得删除列、清空已知提交者或猜测回填历史行；旧二进制运行期间保持审批写入口关闭，直到恢复支持 V029/`WITHDRAWN` 的节点并完成对账。

## 断点续传与对象完整性

`ResumableUploadService` 把 multipart 状态持久化到 `fw_upload_session` 与 `fw_upload_session_part`，并以可信上下文中的租户和用户 ID 绑定用户操作。所有者 ID 只从 `UserRealmProvider.currentUser()` 的单次身份快照取得，是区分大小写且不做 trim、大小写折叠或 Unicode 归一化的不透明字符串；禁止从请求 DTO、Header、metadata、查询参数或浏览器检查点接收。接入值必须非空、最多 256 个 UTF-16 code unit、首尾无 Unicode whitespace，且不含 ISO control 或 FlowWeft 固定拒绝表中的 Unicode format 字符；应用校验与 V024 数据库约束使用同一固定码点表，不受 JDK 8～25 内置 Unicode 版本差异影响。宿主若使用 `Long`、`Int`、UUID 或组合主键，应先在身份 SPI 中稳定转换为字符串，后续不得改变格式。正式 `/fileweft/v1/uploads` 只把会话 ID、公开状态、已确认分片号/长度/时间、过期时间和完成回执返回给浏览器；`tenantId`、`ownerId`、`storageUploadId`、对象路径、存储 ETag、`lastError` 和对象存储凭据始终只能留在服务端。所有该资源的 JSON 成功/失败固定 `private, no-store`、`Pragma: no-cache` 与 `nosniff`，宿主网关不得覆盖为公开缓存。
...
普通上传、文档初版、新版本以及续传的 `start/uploadPart/complete/abort/cleanupExpired` 都会组合数据库状态与非事务型 Storage 副作用，必须作为顶层 Application 边界调用。不要从外层 Spring `@Transactional`、手写 JDBC transaction 或另一个尚未完成的 FlowWeft transaction 调用这些入口。官方 `JdbcApplicationTransaction` 实现 `ApplicationTransactionState`，能在生成 ID、访问仓储或调用 Storage 前拒绝同一 DataSource 的环境事务；不同 DataSource 按引用身份使用独立连接上下文。自定义 transaction 若不实现这个 additive capability 仍保持二进制兼容，但宿主必须自行保证顶层调用；测试辅助方法 `JdbcConnectionContext.withConnection(Connection)` 是匿名绑定，不代表生产宿主事务，Spring `@Transactional` 也不会被它自动感知。
...
网关必须显式允许单个续传分片加少量协议开销，不能沿用 Nginx 的默认 1 MiB 请求体限制。开发编排的页面上传分片上限为 512 MiB，因此 `.docker/nginx.dev.conf` 将 `client_max_body_size` 设为 513 MiB，并对 `/fileweft/` 与 `/api/` 禁用代理请求体缓冲；生产至少应对 `/fileweft/v1/uploads/*/parts/*` 禁用请求缓冲，并根据实际分片上限、磁盘预算、对象存储约束和超时策略作出同等或更严格的配置。仅增大网关限制不会绕过 FlowWeft 的续传会话授权、实际 body 长度校验和对象存储完整性校验。

## Presigned 直传与 OSS 角色装配

直传默认关闭。API 节点负责创建、重签、查询、取消以及经幂等保护的 finalize，不自动执行 staging 删除；恢复过期 provider finalize lease 和删除未完成 staging 必须部署 `fileweft.worker.enabled=true` 的独立 Worker 角色。生产启用前先执行/验证 V034（权威会话）和 V035（原子 FileObject/FileAsset claim），并确认唯一选中的 `StorageAdapter` 实现 `PresignedUploadStorageAdapter`。多个 Storage 候选、缺少 capability、非法租约/批次配置都会启动失败，而不是回退普通上传。

OSS adapter 是可选依赖，Starter 不把它或厂商 SDK强制带给其他存储宿主。应用需要显式依赖 `flowweft-adapter-oss`，并注册恰好一个线程安全、可轮换的 `OssCredentialsProvider` Bean；access key、secret、STS token 和过期时间不得写入 `application.yml`、配置元数据、浏览器存储或 Actuator 可见属性。用于直传的专用 Bucket 必须启用版本控制：finalize 会把已验证对象绑定到不可变 `versionId`，没有版本控制时会安全失败，不能把可覆盖的对象键当作最终 `FileObject` 权威。示例只包含非秘密配置：
...
fileweft:
  storage:
    oss:
      ...
      region: cn-hangzhou
      bucket: company-flowweft-private
      connection-timeout-millis: 5000
      socket-timeout-millis: 120000
      request-timeout-millis: 900000
      max-attempts: 3
      credential-expiry-safety-window-millis: 30000
  upload:
    presigned-enabled: true
    presigned-finalize-grace-millis: 900000
    presigned-claim-lease-millis: 120000
    presigned-maintenance-batch-size: 100
  worker:
    enabled: false # HTTP 节点
```

独立 Worker 使用相同数据库、Storage 与租户安全配置，把 `worker.enabled` 改为 `true`，并按需关闭不属于该角色的 `process-outbox`/`process-tasks`。直传维护指标只在显式 Worker 角色注册并采样，固定为全局低基数 gauge，不使用 tenant、owner、upload ID、对象键或错误作为标签；HTTP 节点不会暴露从未采样却看似健康的零值。Doctor 的 `presigned-upload` 检查只返回聚合计数。`orphanRiskCount > 0` 表示 provider 完成证明已经持久化，但请求在 V035 本地 claim 事务前中断或失败；应让原 owner 使用原 finalize `Idempotency-Key` 在 session expiry 前重试，并检查固定权限、数据库及对象证据，不能由运维手工插入 FileObject/FileAsset 或伪造 claim。

## 下游连接器韧性

Starter 会为默认的交付解析器、兼容的单连接器同步路径和连接器 Doctor 共享同一个保护实例。它在进程内提供硬超时、有限并发/排队和每个连接器独立的熔断状态；Outbox 仍是唯一的重试调度者，不会在一次投递中隐藏式重放请求。

`fileweft.sync.connector-timeout-millis` 只限制一次 FlowWeft 到下游的 RPC 调用；`fileweft.sync.source-access-url-ttl-millis` 则限制交给下游的对象存储访问 URL。两者不能复用：异步下游可能先确认接收、稍后才拉取文件。源 URL 有效期必须为正且不短于 RPC 超时，默认是 15 分钟（900000 毫秒）。异步平台应按其实际取文件队列的最长等待时间加上网络余量配置该值，同时保持预签名 URL 的最小必要权限；不要为了匹配一次 RPC 超时而把 URL 缩短到几十秒。
...
Doctor 的历史 `agent` 检查器仅为旧版兼容保留；`0.0.3` 默认运行时不注册它，Doctor 清单也不展示 Agent。显式启用旧版兼容模式时，该检查器只核对旧 Agent 登记状态，不调用 AI、产生费用或修改文档；这不构成当前产品支持。第三方 AI 连通性检查不得借普通 `DoctorChecker` 绕过 Agent 延后决策并冒充正式 Agent 能力。
