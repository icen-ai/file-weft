...
- **正式公共 HTTP 扩展面**：JDK 8 纯契约、读写/下载、八条生命周期与审批路由、持久化幂等、flat/catalog-aware 安全解析以及 Boot 2/3 MVC 已交付。`revise/publish/offline/restore/archive/submit/approve/reject` 每次重放都会重新授权和检查目录 ACL，route/profile 外调后重验，最终按 idempotency → document → asset → workflow 提交领域状态、审计、Outbox 与稳定 ID。当前仍需把 Doctor、同步状态、日志、插件和系统健康的脱敏投影从丰富 Dev API 提升为正式 v1；不能把对应 Dev Controller 当作公共协议。
