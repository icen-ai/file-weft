...
- **正式公共 HTTP API**：`fileweft-web-api` 与 `fileweft-web-runtime` 已交付 JDK 8 的 v1 纯契约、安全响应分类、cursor 分页、不泄漏领域聚合的读取端口，以及创建草稿/新增版本/改名的最小命令门面；读取端口从可信租户/用户/授权上下文派生目录可见范围。可选 Boot 2/Boot 3 Web Starter 已对齐文档列表、详情、multipart 创建、multipart 新版本与 JSON 改名，安全映射 400/401/403/404/409/503/500、可选 Trace、脱敏响应和安全 `Location`，且 MVC/Spring 仅存在于外层模块。目录模式创建由应用层绑定；目录模式的新增版本和改名会安全失败，直到 Application mutation guard 能在修改锁内重验当前目录 ACL。**完整 HTTP 面仍未闭环**：仍需目录感知修改 guard、持久化请求幂等、授权下载安全头、生命周期/多人审批、Doctor、同步状态与日志的正式映射，以及双租户 Dev v1 E2E；不能把 Dev Controller 当作正式协议。
