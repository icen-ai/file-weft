...
- **正式公共 HTTP API**：`fileweft-web-api` 已交付 JDK 8 的 v1 纯契约（统一 envelope/错误码、文档详情、cursor 分页、审批与 Doctor DTO）以及不泄漏领域聚合的安全文档读取端口。读取端口使用可信租户/用户/授权上下文，目录筛选还会经过宿主目录 ACL；无目录集成时会安全拒绝目录筛选。**但 HTTP 适配器尚未交付**：当前实际 Controller 仍只属于 `fileweft-dev` 的验收应用。仍需以可选 Boot 2/Boot 3 Web Adapter 实现 `/fileweft/v1` 路由、异常/Trace 映射、multipart/下载安全处理、工作流/Doctor 映射、同步状态与日志脱敏读取，以及双租户 MVC/Dev E2E；不能把 MVC 依赖放入 Core、Domain 或默认 Starter。
