...
- **正式公共 HTTP API**：`fileweft-web-api` 与 `fileweft-web-runtime` 已交付 JDK 8 的 v1 纯契约、安全响应分类、cursor 分页和不泄漏领域聚合的读取端口；读取端口从可信租户/用户/授权上下文派生目录可见范围。可选 Boot 2/Boot 3 Web Starter 已实现并对齐 `GET /fileweft/v1/documents` 与 `GET /fileweft/v1/documents/{documentId}`，安全映射 400/401/403/404/503/500、可选 Trace 和脱敏响应，且 MVC/Spring 仅存在于外层模块。**完整 HTTP 面仍未闭环**：仍需 multipart 写入、下载安全头、版本/生命周期/多人审批、Doctor、同步状态与日志的正式映射，以及双租户 Dev v1 E2E；不能把 Dev Controller 当作正式协议。
