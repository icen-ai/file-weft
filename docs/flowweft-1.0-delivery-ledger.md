...
| FW10-050 | 进行中 | retention、legal hold 与安全删除 | 已建立公开治理契约、canonical durable rehydration、provider-neutral CAS/outbox 七阶段运行时及 V041 JDBC 持久化，含 legal-hold 优先、fresh authorization、tenant-bound ID digest、原子状态/outbox、未知结果原操作对账、围栏 worker、Doctor 与指标；继续补适配器、可控时间、对象/索引删除一致性、审计和 PostgreSQL/MySQL/Kingbase 实库测试 |
