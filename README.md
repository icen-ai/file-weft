...
正式写接口所需的持久化请求幂等地基已经使用 `V020` 独立交付：原始 `Idempotency-Key` 只在内存中校验，数据库仅保存租户作用域摘要，并把当前用户、动作、资源/子资源和 typed-command 指纹绑定到第一次成功结果。它采用 idempotency → document → asset → workflow 的固定锁序，使业务状态、审计、Outbox 与安全资源 ID 同事务提交。该地基尚不等于生命周期 HTTP 已开放；各用例仍需完成内部事务钩子和 Boot 2/3 协议测试，不能在 Controller 外包一层事务把目录、路由或交付 SPI 拖进数据库事务。
