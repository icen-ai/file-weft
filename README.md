...
正式写接口所需的持久化请求幂等地基已经使用 `V020` 独立交付：原始 `Idempotency-Key` 只在内存中校验，数据库仅保存租户作用域摘要，并把当前用户、动作、资源/子资源和 typed-command 指纹绑定到第一次成功结果。它采用 idempotency → document → asset → workflow 的固定锁序，使业务状态、审计、Outbox 与安全资源 ID 同事务提交。`revise`、`publish`、`offline`、`restore`、`archive` 已提供 flat 与 catalog-aware 两种幂等 Application 边界；每次重放仍重新执行当前授权和目录 ACL，Publish 仅在未命中后于事务外冻结交付策略。该能力尚不等于生命周期 HTTP 已开放：工作流提交/审批仍需接入同一事务协议，正式 Controller 与 Boot 2/3 MVC 契约也尚未发布，不能在 Controller 外包一层事务把目录、路由或交付 SPI 拖进数据库事务。
