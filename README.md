...
> **Agent 产品决策：** `0.0.2` 不提供 Agent 产品能力。Agent 将重新设计，
> 但无限期延后，最早只能在 `1.0.0` 正式发布后重新评估；这不构成对
> `1.x`、下一版本或任何版本的交付承诺。现有 `fileweft-agent`、Agent
> SPI/公共 ABI 以及 V012/V026 中相关表和列只为兼容保留。默认运行时、
> Doctor/插件清单、正式 HTTP 和 `fileweft-dev` 均不暴露 Agent；旧文档与
> `0.0.1` 发布记录中的 Agent 内容仅作为历史记录。
...
FileWeft Starter 不会传递引入 `spring-boot-starter-jdbc`、HikariCP 或其他连接池；数据库连接、池实现、容量与凭据轮换都归宿主所有。上面的推荐组合让 Spring Boot 根据 `spring.datasource.*` 创建唯一的 `DataSource`；已有自定义池的宿主可改为提供自己的 `DataSource` Bean。Boot 2 项目将 FileWeft 坐标中的 `boot3` 改为 `boot2`，宿主 JDBC Starter 坐标不变。
...
本仓库的 `v0.0.2` 发布身份对应 Maven 坐标 `ai.icen:*:0.0.2`。只有受保护的 `v0.0.2` 标签流水线全部成功，且公开 CNB Maven 仓库能够被匿名冷缓存重新解析后，才应消费这些稳定坐标；不能仅凭源码、标签或本地制品推断远端发布已经完成。本机验证仍可先执行 `.\gradlew.bat installReleaseToMavenLocal`，并在消费项目中启用 `mavenLocal()`。
...
`v0.0.2` 对应源码为 PostgreSQL、MySQL 与 KingbaseES 各自提供完整的 28 个迁移 V001–V028；V026 决策证据仍有独立升级专章，V027 增加 Worker claim-order 索引，V028 将全部 MySQL FileWeft 业务表统一为 NO PAD 的 `utf8mb4_0900_bin`。受保护标签成功发布后，这三套 V001–V028 即构成不可改写的 `0.0.2` 发布资源。作为历史边界，`v0.0.1` 只发布了 PostgreSQL V001–V025；MySQL、KingbaseES 和 V026–V028 首次进入 `v0.0.2` 发布合同。MySQL 支持范围仅是原生 MySQL 8.x 的 8.0.17+，本轮实库证据为 8.0.46，不包含 MariaDB 或 MySQL 9。
