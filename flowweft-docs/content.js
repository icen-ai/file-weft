...
export const ui = {
  "en": {
    ...
    "searchPlaceholder": "Search FlowWeft",
...
export const pages = {
  "getting-started/introduction": {
    ...
    "en": {
      ...
      "lead": "Understand what FlowWeft is, where its responsibilities end, and which integration path fits your team.",
  ...
  "getting-started/installation": {
    ...
    "en": {
      ...
      "lead": "Add FlowWeft to your build, align Spring Boot generations, and verify the dependency tree.",
      "file": "pages/en/getting-started/installation.md"
    },
    "zh": {
      "nav": "安装",
      "title": "安装 0.0.3 版本线",
      "lead": "将 FlowWeft 加入构建，对齐 Spring Boot 代际，并验证依赖树。",
  ...
  "getting-started/quickstart": {
    ...
    "en": {
      "nav": "Quick start",
      "title": "Quick start",
      "lead": "Run a complete FlowWeft endpoint on your laptop with a minimal Spring Boot host and a few curl commands.",
      "file": "pages/en/getting-started/quickstart.md"
    },
    "zh": {
      "nav": "快速开始",
      "title": "快速开始",
      "lead": "用最小的 Spring Boot 宿主和几个 curl 命令，在笔记本上跑通一个完整的 FlowWeft 端点。",
  ...
  "concepts/module-boundaries": {
    ...
    "zh": {
      "nav": "模块边界",
      "title": "模块边界",
      "lead": "为什么 FlowWeft 要把代码拆到多个模块？因为一旦存储 SDK 泄漏到领域层，框架就变得难以升级和替换。本页说明依赖方向、每个模块的职责，以及如何在不过线的前提下新增适配器。",
  ...
  "concepts/lifecycle-delivery": {
    ...
    "en": {
      ...
      "lead": "A published document is not a single checkbox: it is a chain of evidence that must survive review, partial downstream failures, and later removal. This page explains FlowWeft's state machine, how delivery to multiple systems is tracked, and why generation fencing matters.",
      "file": "pages/en/concepts/lifecycle-delivery.md"
    },
    "zh": {
      "nav": "生命周期与交付",
      "title": "生命周期与交付",
      "lead": "“已发布”不是单个布尔值，而是一条必须经受审批、下游部分失败和后续撤回考验的证据链。本页说明 FlowWeft 的状态机、如何按目标跟踪多下游交付，以及代次围栏为何重要。",
  ...
  "concepts/security-model": {
    ...
    "en": {
      ...
      "lead": "FlowWeft does not guess who you are or what you can do. It delegates identity, tenant, and authorization to the host and treats a missing or ambiguous boundary as a denial. This page explains the three provider contracts, the fail-closed rule, and what stays out of public responses.",
      "file": "pages/en/concepts/security-model.md"
    },
    "zh": {
      "nav": "安全模型",
      "title": "故障关闭安全模型",
      "lead": "FlowWeft 不会猜测调用者是谁、能做什么。它把身份、租户和授权委托给宿主，并把缺失或歧义的安全边界视为拒绝。本页说明三个提供者契约、故障关闭规则，以及公共响应中不会暴露什么。",
  ...
  "guides/spring-boot": {
    ...
    "en": {
      ...
      "title": "Assemble FlowWeft into Spring Boot",
      "lead": "Learn how to embed FlowWeft as an infrastructure library inside your Spring Boot application while keeping full control over authentication, DataSource ownership, and gateway policy.",
  ...
  "guides/workflows-uploads": {
    ...
    "en": {
      ...
      "lead": "Understand how FlowWeft handles long-running work—document review, multipart uploads, and generic background tasks—without weakening transaction boundaries or leaking storage internals.",
  ...
  "guides/catalog-provider": {
    ...
    "en": {
      ...
      "lead": "Bind FlowWeft documents to your host folder tree while keeping control of directory paths, ACLs, and display names.",
      "file": "pages/en/guides/catalog-provider.md"
    },
    "zh": {
      "nav": "目录 Provider",
      "title": "实现目录 Provider",
      "lead": "将 FlowWeft 文档绑定到宿主目录树，同时保留对目录路径、ACL 和显示名称的控制。",
  ...
  "guides/storage-adapter": {
    ...
    "en": {
      ...
      "lead": "Add a new object backend by implementing the StorageAdapter SPI. FlowWeft builds object names and metadata; your adapter only materializes bytes.",
  ...
  "architecture/consistency": {
    ...
    "en": {
      ...
      "lead": "FlowWeft does not promise a distributed transaction across PostgreSQL, object storage, and downstream systems. It keeps every local state change atomic and makes remote convergence observable, retryable, and safe to reason about.",
      "file": "pages/en/architecture/consistency.md"
    },
    "zh": {
      "nav": "一致性模型",
      "title": "本地事务原子，远程状态显式收敛",
      "lead": "FlowWeft 不承诺跨 PostgreSQL、对象存储和多个下游系统的分布式事务。它保证每一次本地状态变更的原子性，并让远端收敛过程可观测、可重试且易于推理。",
  ...
  "architecture/security": {
    ...
    "en": {
      ...
      "lead": "FlowWeft assembles a capability only when its complete security boundary is present. Missing context, ambiguous providers, or unverified custom persistence make the operation unavailable instead of silently widening access.",
      "file": "pages/en/architecture/security.md"
    },
    "zh": {
      "nav": "安全架构",
      "title": "所有边界默认拒绝",
      "lead": "FlowWeft 只在完整安全边界存在时才装配能力。上下文缺失、Provider 歧义或未经验证的自定义持久化都会让操作不可用，而不是静默扩大访问范围。",
  ...
  "reference/spi": {
    ...
    "en": {
      "nav": "SPI index",
      "title": "SPI surface",
      "lead": "FlowWeft is built around contracts, not concrete vendors. The SPI lets you plug in identity, storage, catalogs, workflows, connectors, diagnostics, and background tasks without changing the framework internals.",
  ...
  "reference/configuration": {
    ...
    "en": {
      ...
      "lead": "FlowWeft production defaults are conservative: schema validation, no implicit tenant, no local storage, and no hidden migrations. Enable every fallback and runtime role deliberately.",
      "file": "pages/en/reference/configuration.md"
    },
    "zh": {
      "nav": "配置",
      "title": "配置参考",
      "lead": "FlowWeft 的生产默认值是保守的：校验 schema、不隐式选择租户、不使用本地存储、不自动迁移。每个 fallback 和运行角色都必须显式开启。",
  ...
  "reference/error-codes": {
    ...
    "en": {
      ...
      "lead": "When something goes wrong, FlowWeft returns a stable machine-readable code inside the JSON envelope. This page explains each code and the recovery action to take.",
      "file": "pages/en/reference/error-codes.md"
    },
    "zh": {
      "nav": "错误码",
      "title": "稳定错误码",
      "lead": "当请求失败时，FlowWeft 会在 JSON 外层里返回一个稳定的机器可读错误码。本页说明每个错误码的含义以及恢复操作。",
  ...
  "reference/domain-model": {
    ...
    "en": {
      ...
      "lead": "FlowWeft separates business concepts from infrastructure. This page shows the core entities, their responsibilities, and how they connect to storage, workflow, and delivery.",
      "file": "pages/en/reference/domain-model.md"
    },
    "zh": {
      "nav": "领域模型",
      "title": "核心领域模型",
      "lead": "FlowWeft 把业务概念与基础设施分离。本页展示核心实体、职责以及它们如何与存储、工作流和交付产生关联。",
  ...
  "operations/deployment": {
    ...
    "en": {
      ...
      "lead": "Run one validated FlowWeft artifact as four intentionally different roles — API, Outbox worker, Task worker and migration job — so each process gets only the credentials, configuration and blast radius it needs.",
      "file": "pages/en/operations/deployment.md"
    },
    "zh": {
      "nav": "生产部署",
      "title": "按运行角色部署",
      "lead": "将同一份已验证的 FlowWeft 制品以四种有意区分的角色运行——API、Outbox Worker、任务 Worker 与迁移 Job——让每个进程只拥有它所需的凭据、配置和爆炸半径。",
  ...
  "operations/doctor-observability": {
    ...
    "en": {
      ...
      "lead": "Diagnose FlowWeft without guessing: Doctor runs safe, authorized checks; metrics expose bounded trends; audit logs and trace IDs carry resource evidence without leaking high-cardinality labels.",
      "file": "pages/en/operations/doctor-observability.md"
    },
    "zh": {
      "nav": "Doctor 与可观测性",
      "title": "从证据出发运维",
      "lead": "无需猜测即可诊断 FlowWeft：Doctor 执行安全、已授权的检查；指标展示有界趋势；审计日志与 Trace ID 承载资源证据，同时避免高基数标签泄漏。",
  ...
  "operations/migrations-release": {
    ...
    "en": {
      ...
      "lead": "FlowWeft owns a namespaced Flyway location and history table. Release safely by validating schema compatibility, real infrastructure paths, SBOM integrity and reproducible dependency state.",
  ...
  "operations/troubleshooting": {
    ...
    "zh": {
      "nav": "故障排查",
      "title": "诊断常见症状",
      "lead": "当 FlowWeft 行为异常时，按结构化清单操作——先检查 Doctor、指标、Outbox 状态与 Trace ID，再改动代码或配置。",
  ...
  "extensions/plugins": {
    ...
    "en": {
      ...
      "lead": "Learn how to package FlowWeft extensions into a single plugin artifact so operators can discover connectors, storage adapters, Doctor checkers, and task handlers without changing core code.",
      "file": "pages/en/extensions/plugins.md"
    },
    "zh": {
      "nav": "插件开发",
      "title": "编写生产级插件",
      "lead": "学习如何将 FlowWeft 扩展打包成单个插件制品，让运维人员无需修改核心代码就能发现连接器、存储适配器、Doctor 检查器和任务处理器。",
  ...
  "project/contributing": {
    ...
    "zh": {
      "nav": "参与贡献",
      "title": "贡献功能，不侵蚀地基",
      "lead": "本页说明如何在为 FlowWeft 添加代码、测试与文档的同时，保持 Core → SPI → Domain → Application → Persistence → Starter → Adapter 的分层完整，让框架保持可扩展且对 Java 友好。",
  ...
  "project/security": {
    ...
    "en": {
      ...
      "lead": "This page explains how to disclose a suspected vulnerability in FlowWeft so it can be investigated and fixed before it becomes public knowledge.",
      "file": "pages/en/project/security.md"
    },
    "zh": {
      "nav": "安全",
      "title": "私密报告安全问题",
      "lead": "本页说明如何披露 FlowWeft 的疑似漏洞，使其在公开之前得到调查和修复。",
  ...
  "project/roadmap": {
    ...
    "zh": {
      "nav": "开发待办",
      "title": "用证据推进的开发路线图",
      "lead": "这份待办是后续 FlowWeft 开发的交接契约。只有仓库或指定真实环境中存在对应证据，版本才能从计划转为完成；仅有实现、局部绿测或书面意图，都不能单独作为完成依据。",
  ...
  "project/faq": {
    ...
    "en": {
      ...
      "lead": "This page answers the most common questions about FlowWeft's purpose, architecture, tenancy, storage and release policy, with pointers to deeper documentation.",
  ...
  "architecture/event-driven": {
    ...
    "en": {
      ...
      "lead": "FlowWeft turns every external side effect into a durable, tenant-scoped event. Workers process those events at-least-once, connectors converge downstream systems, and operators can observe the whole pipeline without peeking at internals.",
      "file": "pages/en/architecture/event-driven.md"
    },
    "zh": {
      "nav": "事件驱动交付",
      "title": "基于 Outbox 与 Worker 的事件驱动交付",
      "lead": "FlowWeft 把每一个外部副作用都变成持久、带租户作用域的事件。Worker 以至少一次语义处理这些事件，连接器收敛下游系统，运维人员无需窥探内部即可观测整条流水线。",
