...
如果其中任何一步执行到一半失败，朴素的分布式事务要么锁住所有系统，要么留下不一致状态。FlowWeft 通过让本地数据库成为唯一真相源，并把每一次远程调用都建模为最终一致的投影，来同时避免这两种结果。

## 核心思想：本地原子性优先

FlowWeft 只信任本地 PostgreSQL 事务，其他一切都被建模为框架后续收敛的持久事件。
...
> **NOTE**
> FlowWeft 绝不在数据库事务中调用下游连接器。事务只写入 Outbox 记录，异步 Worker 随后执行远程调用。
...
上传会先于业务记录产生字节。FlowWeft 在能证明这些字节未被引用时进行补偿：
...
文档发布时，FlowWeft 会根据当前同步配置向所有目标扇出：

| 必达目标 | 可选目标 | 最终文档状态 |
| --- | --- | --- |
| 全部成功 | 任意结果 | `PUBLISHED` |
| 一个失败且可重试 | 不决定结果 | `SYNC_ERROR`；有限自动重试耗尽后事件/目标进入 `FAILED`，文档继续保持 `SYNC_ERROR`，直到人工重排成功 |
| 一个永久失败 | 不决定结果 | 事件/目标进入 `FAILED`；文档保持 `SYNC_ERROR`，直到运维修复并重排该交付 |
| 所有必达目标成功 | 一个可选目标失败 | 仍为 `PUBLISHED`；可选目标失败会被记录供运维排查 |

> **NOTE**
> FlowWeft 不会回滚已经成功的连接器。交付是追加写入：成功目标保持发布；失败目标只重试到配置上限，随后需要运维排查并重排。
...
**Q: FlowWeft 是否支持对象存储与 PostgreSQL 的两阶段提交？**

不支持。FlowWeft 先提交 PostgreSQL，再收敛对象存储和连接器。这避免了在远程系统上持有锁，也让分区场景下架构仍可运行。
...
- [安全架构](/architecture/security) — 边界不完整时 FlowWeft 如何失败关闭。
