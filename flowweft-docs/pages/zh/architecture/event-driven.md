...
FlowWeft 通过把每个外部副作用移到 Outbox 和一组异步 Worker 之后来解决这些问题。

## 事件流

发布命令不会调用连接器，它只写入事件，然后由 Worker 池收敛下游状态：
...
某些副作用太长或太重要，不能内联执行。FlowWeft 把它们表示为任务，并路由给注册的 `FileWeftTaskHandler` Bean：
...
框架暴露了计数器与 Gauge，让你在不添加租户或文档标签的情况下观测流水线：
...
> **NOTE**
> 标签中不能包含 `tenantId`、文档 ID 或用户 ID。FlowWeft 保持指标基数较低，避免暴露高基数或敏感维度。
