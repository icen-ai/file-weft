...
2. 在 Spring Boot 3 项目中引入已完成远端验证的 FlowWeft 0.0.3 坐标；
3. 提供 FlowWeft 要求的三个 SPI bean；
4. 启用开发 fallback；
...
运行本地 PostgreSQL 容器，创建 FlowWeft 使用的数据库和用户：
...
`documentId` 与 `versionId` 来自已提交的领域聚合。FlowWeft 有意不在命令后执行第二次查询，因此即使调用方没有读权限，响应也会成功。
