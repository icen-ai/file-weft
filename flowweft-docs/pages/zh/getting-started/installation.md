...
这页给出稳定版 `v0.0.3` 的 Maven 坐标、JDK 与 Spring Boot 约束，以及一个快速命令来确认正确的 artifacts 已进入你的 classpath。固定提交 `dbf2a50fbca41e2ac5b5cf18bb44f9287c153637` 已在 CNB 构建 `cnb-cl8-1jtgih45j` 完成 12/12 发布流水线和全部 19 个坐标的匿名远端回读，可以使用下面的稳定坐标。
...
> [!WARNING]
> 不要在同一应用中混用 Boot 2 与 Boot 3 的 FlowWeft Starter。Web API 契约一致，但底层 Spring 依赖不同。

## Maven 坐标

FlowWeft 的 Maven group 为 `ai.icen`，JVM 包名为 `ai.icen.fw`。早期 `com.fileweft:*:0.0.1` 试用坐标已撤回，不再受支持。
...
> [!NOTE]
> Runtime starter 是便利包，包含持久化、Worker、应用服务与观测适配器。Web starter 额外暴露正式的 `/fileweft/v1` HTTP 接口。若需要可运行的 REST API，两者都加。`spring-boot-starter-jdbc` 属于宿主依赖，其版本应由宿主的 Spring Boot Gradle 插件、Parent 或 BOM 管理；FlowWeft 不传递选择 HikariCP 或其他连接池。

### Boot 2 的 Kotlin 运行时对齐

Boot 2.7 BOM 默认把 Kotlin 管理为 1.6.21，低于 FlowWeft 使用的 2.1.21；纯 Java 宿主运行 FlowWeft 字节码时同样需要对齐。使用 Spring Dependency Management 时设置 `extra["kotlin.version"] = "2.1.21"`，Maven 设置 `<kotlin.version>2.1.21</kotlin.version>`；使用原生 Gradle platform 时同时导入 `org.jetbrains.kotlin:kotlin-bom:2.1.21` 或采用等价的显式解析规则。普通 Kotlin BOM 不能覆盖 Boot 2 `enforcedPlatform`；请用 `dependencyInsight` 确认 `kotlin-stdlib` 最终为 2.1.21。
...
现在可以。稳定版 `v0.0.3` 固定于 `dbf2a50fbca41e2ac5b5cf18bb44f9287c153637`；CNB 构建 `cnb-cl8-1jtgih45j` 已完成 12/12 流水线，并匿名回读全部 19 个坐标。上线时仍须遵守下文的 V029 升级边界，不能把制品可用误解为允许不安全的滚动迁移。

**Q: 是否必须同时引入 runtime starter 和 web starter？**

如果只在 Spring 应用内部通过 Kotlin/Java 调用 FlowWeft，runtime starter 足够。若需要 `/fileweft/v1` REST 端点，再加对应 web starter。
