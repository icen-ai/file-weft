...
路由解析在 FlowWeft 数据库事务外执行，以便宿主策略安全地查询外部系统。

## DeliveryTarget

文档发布时，FlowWeft 会为当前同步 profile 中的每个目标创建一个交付尝试。
...
领域层不直接调用外部系统，而是发出 Outbox 事件。例如，当文档下线或归档时，FlowWeft 会写入 `document.delivery.target.removal.requested` 事件，让每个下游目标异步清理。
...
> [!WARNING]
> 不要信任请求参数中的 `tenantId`。FlowWeft 始终向 `TenantProvider` 索取当前可信租户。
