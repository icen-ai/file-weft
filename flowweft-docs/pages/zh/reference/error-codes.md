...
| `RANGE_NOT_SUPPORTED` | FlowWeft 下载不支持 Range 请求。 | 请求完整资源。 |
...
| `INTERNAL_ERROR` | FlowWeft 内部发生意外失败。 | 退避重试，持续出现则联系运维。 |
...
用同一个 `Idempotency-Key` 重放命令时，如果指纹匹配，FlowWeft 会返回原始结果。如果 key 相同但命令不同，则返回 `CONFLICT`：
