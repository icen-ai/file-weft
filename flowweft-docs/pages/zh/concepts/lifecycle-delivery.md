...
控制器不会直接修改状态字段。调用方发送命令，FlowWeft 应用领域规则。
...
一份文档通常需要到达多个下游系统：合规归档、检索索引、内容分发网络。FlowWeft 为每个目标单独跟踪。

| 目标类型 | 是否必达 | 失败影响 |
|---|---|---|
| 必达 | 是 | 阻止进入 `PUBLISHED`；文档进入 `SYNC_ERROR`，自动重试有上限，耗尽后需要运维重排 |
| 可选 | 否 | 文档仍可发布；失败被记录，自动重试到同一上限后需要运维重排 |

保护下游可靠性的规则：

1. **所有必达目标成功**后，文档才会变成 `PUBLISHED`。
2. **可选目标失败不会阻塞发布。** 文档发布后，失败目标仍可重试。
3. **不会回滚已成功目标。** 如果后面的目标失败，FlowWeft 不会撤销之前成功的目标。
...
FlowWeft 从不在写入文档状态的数据库事务中调用下游连接器。它会把 Outbox 事件写入同一份 PostgreSQL 事务，再由 Worker 异步交付。
...
当文档被下线或归档时，FlowWeft 会为每个已交付目标写入 `document.delivery.target.removal.requested` 事件。失败的移除同样可以这样重试：
...
**Q：没有配置目标也能发布文档吗？**
不能。每个可发布 Profile 都必须至少包含一个目标和一个必达目标。FlowWeft 不会把发布静默退化为“仅本地成功”；请注册对应的必达连接器，或选择另一份有效 Profile。

**Q：必达目标永久宕机怎么办？**
文档会停留在 `SYNC_ERROR`。永久失败或有限自动重试耗尽后，事件与目标进入 `FAILED`，自动重试停止。连接器恢复健康后，拥有 `document:delivery:retry` 权限的运维人员通过交付重试端点重排该目标。
...
- [Outbox 模式](./outbox.md)——FlowWeft 如何避免 PostgreSQL 与外部系统之间的双写。
