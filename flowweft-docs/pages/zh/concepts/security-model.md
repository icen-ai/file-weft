...
> [!NOTE]
> 用户 ID 是不透明字符串。FlowWeft 保留大小写、不裁剪空白、拒绝 ISO 控制字符。请在宿主侧完成用户 ID 归一化后再交给 FlowWeft。
...
- [插件](../extensions/plugins.md)——用可信的进程内插件扩展 FlowWeft。
