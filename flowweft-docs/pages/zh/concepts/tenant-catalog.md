...
FlowWeft 从不信任来自请求参数、路径变量或查询字符串的租户 ID。宿主从自己的认证上下文中解析租户，并通过 `TenantProvider` 提供。
...
FlowWeft 不存储目录层级。浏览器只提交一个受约束的不透明 `folderId`。FlowWeft 使用可信的租户、用户与操作上下文询问宿主目录提供者，然后将返回的 canonical ID 保存为元数据。
...
目录验证通过后，FlowWeft 将 canonical ID 写入资产元数据：
...
对象存储键中绝不包含目录 ID。宿主目录改名或换父级时，FlowWeft 不需要移动任何字节。

## 03. 不会静默降级

目录模式开启后，所有需要目录的变更操作都必须经过提供者。如果提供者无法确认该变更是安全的，FlowWeft 会返回 `FEATURE_UNAVAILABLE`，不会降级到租户级写入路径。
...
| 提供者确认访问 | FlowWeft 继续执行并保存 `catalog.folder-id` |
...
5. FlowWeft 保存文件，并在元数据中写入 `catalog.folder-id=folder-42`。
...
**Q：为什么不把目录 ID 放到对象键里？**
因为目录树归宿主所有。如果目录改名或移动，FlowWeft 就要重写所有对象键。把目录放在元数据里，可以让 FlowWeft 与宿主目录变更解耦。

**Q：如果宿主目录暂时不可用怎么办？**
FlowWeft 会把缺少安全变更能力视为 `FEATURE_UNAVAILABLE`，不会继续用租户级默认目录。
