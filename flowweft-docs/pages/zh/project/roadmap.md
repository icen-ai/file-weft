...
<aside class="callout" data-mark="已批准"><div><strong>1.0 包含重新设计的 Agent、权限检索与产品控制台</strong><p>0.0.2 与 0.0.3 不提供 Agent 仍是历史事实；旧 <code>fileweft-agent</code>、Agent SPI/公共 ABI 与 V012/V026 只作兼容，不改义复用。后续开发直接收敛 1.0，通过增量 API/runtime 实现模型与工具 Provider、持久编排、强制 ACL 前置过滤和权威复核。</p><p>核心只内置安全文件名匹配；全文、向量、混合、重排、抽取和模型能力全部走 SPI。1.0 只维护 RustFS、Dify 知识库和阿里云 OSS 三个参考集成，不拥有宿主目录 CRUD。</p></div></aside>

<h2>1.0 里程碑 A · 生产供应链</h2>

<p>该里程碑证明制品可被消费、验证并从源码追溯到仓库；它是 1.0 发布门禁，不要求先发布 0.1.0。</p>
...
<h2>1.0 里程碑 B · 宿主集成与可观测性</h2>

<p>该里程碑扩大可复用测试套件、真实 OTel 证据和生产 Doctor；MySQL 8 与 KingbaseES 的既有实库证据仍保持其历史版本边界。</p>
...
<h2>1.0 里程碑 C · 规模化运营</h2>

<p>该里程碑让 FlowWeft 在高容量、受监管环境中可运维，并具备明确恢复目标。</p>
...
<h2>1.0 里程碑 D · 参考集成与智能产品面</h2>

<p>该里程碑交付三个有真实服务证据的参考集成、新版 Agent/检索运行时和独立产品控制台。</p>

<table class="comparison-table">
<thead><tr><th>交付内容</th><th>验收证据</th><th>禁止冒充完成的边界</th></tr></thead>
<tbody>
<tr>
<td>RustFS（经 S3 adapter）、Dify 知识库、阿里云 OSS 三个参考集成；Provider-neutral Agent/检索；覆盖全部正式能力的 Next.js Console。</td>
<td>三个隔离真实环境分别验证认证、超时、重试、幂等、删除、Doctor 与兼容矩阵；检索安全门禁覆盖 tenant/ACL 前置过滤和权威复核；Console 覆盖双语、登录、文档、审批、同步、Doctor、审计、Agent、工具确认和配置 E2E。</td>
<td>不交付 ESE/AppBuilder 等易变厂商适配器；模拟器不能证明真实服务；无法证明强制 ACL 前置过滤的 Dify 模式只能作为 delivery sink/演示，不能冒充生产 Retriever。</td>
</tr>
</tbody>
</table>
...
<tbody>
<tr>
<td>公共 API/ABI 冻结；新版 Agent、权限检索与 Console；受支持迁移升级矩阵；安全审计；完整运维、集成与兼容文档。</td>
