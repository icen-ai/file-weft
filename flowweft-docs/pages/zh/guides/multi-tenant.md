...
FlowWeft 从设计之初就面向多租户部署。租户隔离不是某个配置开关，而是跨查询、存储路径、事件、任务、日志和缓存的横切职责。本页说明如何提供租户上下文，以及为什么不能信任请求参数中的租户 ID。

## 1. 提供租户上下文

租户身份的唯一稳定来源是 `TenantProvider` SPI。FlowWeft 在需要当前租户时都会调用它。
...
> [!WARNING]
> 不要在 FlowWeft 内部直接读取 `X-Tenant-Id` 等客户端头。只信任由认证层建立的上下文。

## 2. 数据库查询隔离

所有 FlowWeft 业务表都包含 `tenant_id`。仓储对每个查询都按当前租户上下文过滤，聚合和计数查询也不例外。
...
| 审计日志 | 查询端点按当前租户上下文过滤。 |
| 缓存 | 缓存键必须包含租户 ID。FlowWeft 不提供跨租户共享缓存。 |
...
可以。FlowWeft 使用 `tenant_id` 行级隔离。使用一个 schema 还是多个 schema 是宿主的运维选择。
