...
2. Email it to **support@icen.ai** with the subject prefix `[SECURITY] FlowWeft`.
...
- Affected FlowWeft version and module (`fileweft-core`, `fileweft-web-api`, a specific starter, etc.).
...
```text
Subject: [SECURITY] FlowWeft - possible tenant isolation bypass in workflow decisions

Affected version: ai.icen:fileweft-web-api:0.0.3
...
The FlowWeft maintainers follow a coordinated disclosure process:
...
**Does FlowWeft offer a bug bounty?**
There is no formal bounty program at this time. Responsible disclosure is still appreciated and acknowledged.

## Next steps

- Read the [security architecture](architecture/security) page to understand how FlowWeft fails closed at boundaries.
