...
FlowWeft is a Kotlin/JVM infrastructure framework for enterprise file intelligence. It handles document lifecycles, approvals, multi-target delivery, storage abstraction, diagnostics and audit — but it is not a simple upload module, a business document system, a Dify/ESE wrapper or a cloud storage SDK.

### Is FlowWeft a drop-in file upload widget?

No. FlowWeft provides the foundation; your host supplies identity, tenant, authorization, folder tree and business rules through SPI implementations.
...
### Does FlowWeft create tenants?

No. FlowWeft consumes tenant context from your `TenantProvider`. The framework applies that context to queries, storage paths, events, tasks, logs and caches.
...
> FlowWeft never trusts a `tenantId` sent by the caller. It always resolves the current tenant through the configured `TenantProvider`.

## Storage

### Does FlowWeft replace S3 or MinIO?

No. FlowWeft abstracts storage through `StorageAdapter` so you can keep using S3, MinIO, OSS or a custom backend without leaking vendor details into your domain.

### Can I serve files directly from object storage?

FlowWeft does not expose presigned storage URLs through the public HTTP API. Authorized downloads are binary streams returned by the application with `attachment`, `nosniff` and `private, no-store` headers.
