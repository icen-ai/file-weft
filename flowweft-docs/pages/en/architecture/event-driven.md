...
FlowWeft solves this by moving every external side effect behind an Outbox and a set of asynchronous workers.
...
Some side effects are too long or too important to run inline. FlowWeft represents them as tasks and routes them to registered `FileWeftTaskHandler` beans:
...
Connectors must also implement health checks so FlowWeft can degrade gracefully:
...
> **NOTE**
> Labels must not include `tenantId`, document IDs, or user IDs. FlowWeft keeps metrics cardinality low and avoids exposing high-cardinality or sensitive dimensions.
