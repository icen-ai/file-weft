...
Enterprise file infrastructure sits between untrusted callers and sensitive bytes. A small mistake in provider resolution, a leaked object key, or an over-shared diagnostic message can expose one tenant's data to another. FlowWeft's default posture is to refuse service when any required boundary is unclear.

## Fail-closed capability assembly

FlowWeft exposes a capability only after it can identify a single, unambiguous provider for every boundary that affects security:
...
If two catalog providers or two lifecycle candidates are both visible, FlowWeft does **not** pick one by guessing, by `@Primary`, or by classpath order when that choice could change security semantics. The operation reports an error and stops.

> **WARNING**
> Do not rely on Spring `@Primary` to resolve security-sensitive providers. FlowWeft considers multiple eligible candidates an assembly failure.
...
Without these guarantees, FlowWeft will expose read-only projections and reject commands rather than risk concurrent mutation or cross-tenant writes.
...
> **NOTE**
> Downloads are returned as binary streams with `attachment`, `nosniff`, and `private, no-store` caching headers. FlowWeft does not expose storage URLs, Range negotiation, or ETags to API clients.

## Plugins are trusted code

A FlowWeft plugin runs in the same JVM as the host application. It shares the classloader, permissions, and memory space. There is no in-process sandbox.
...
FlowWeft provides a fixed-tenant fallback and local storage fallback for single-tenant development only. They are not a production multi-tenant solution. Production deployments must supply real `TenantProvider`, `UserRealmProvider`, and `AuthorizationProvider` implementations.

**Q: What happens if my custom repository forgets to filter by tenant?**

FlowWeft's SPI contracts require tenant-scoped queries. If a custom persistence implementation cannot prove tenant isolation, guarded writes remain disabled and the application logs an assembly error at startup.
...
- [Consistency model](/architecture/consistency) — how FlowWeft keeps local state atomic while converging remote systems.
