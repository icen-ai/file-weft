...
If any step fails halfway through, a naive distributed transaction would either lock all three systems or leave them inconsistent. FlowWeft avoids both by making the local database the single source of truth and treating every remote call as an eventually consistent projection.

## Core idea: local atomicity first

The only transaction FlowWeft trusts is the local PostgreSQL transaction. Everything else is modeled as a durable event that the framework converges later.
...
> **NOTE**
> FlowWeft never invokes a downstream connector inside a database transaction. The transaction only writes the Outbox record; workers execute the remote call afterwards.
...
Uploads create bytes before the business record exists. FlowWeft compensates storage when it can prove the bytes are unreferenced:
...
When a document is published, FlowWeft fans out to every target in the active sync profile:
...
> **NOTE**
> FlowWeft never rolls back a connector that already succeeded. Delivery is append-only: successful targets stay published; failed targets retry only to the configured limit and then require operator triage/requeue.
...
**Q: Does FlowWeft support two-phase commit across object storage and PostgreSQL?**

No. FlowWeft commits PostgreSQL first and converges object storage and connectors afterwards. This avoids holding locks on remote systems and keeps the architecture operational under partitions.
...
- [Security architecture](/architecture/security) — how FlowWeft fails closed when boundaries are incomplete.
