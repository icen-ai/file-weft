...
FlowWeft never trusts a tenant ID that arrives in a request parameter, path variable or query string. The host resolves the tenant from its own authenticated context and supplies it through `TenantProvider`.
...
FlowWeft does not store folder hierarchies. The browser submits only a bounded opaque `folderId`. FlowWeft asks the host catalog provider with trusted tenant, user and operation context, then stores the returned canonical folder ID as metadata.
...
After the folder is validated, FlowWeft writes the canonical ID into the asset metadata:
...
Object storage keys never contain the folder ID. A rename or re-parent operation in the host catalog does not require FlowWeft to move any bytes.

## 03. No silent fallback

When catalog mode is enabled, every mutation that needs a folder must pass through the provider. If the provider cannot confirm that the mutation is safe, FlowWeft returns `FEATURE_UNAVAILABLE`. It never falls back to a tenant-wide write path.
...
| Provider confirms access | FlowWeft proceeds and stores `catalog.folder-id` |
...
5. FlowWeft stores the file and writes `catalog.folder-id=folder-42` in metadata.
...
Because the host owns the tree. Renaming or moving a folder would force FlowWeft to rewrite every object key. Keeping the folder in metadata decouples FlowWeft from host catalog changes.

**Q: What if the host catalog is temporarily unavailable?**
FlowWeft treats a missing safe-mutation capability as `FEATURE_UNAVAILABLE`. It does not proceed with a tenant-wide default folder.
