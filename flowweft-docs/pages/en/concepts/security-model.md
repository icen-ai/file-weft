...
FlowWeft draws three security boundaries around every operation:
...
The host tells FlowWeft who is calling and which tenant the call belongs to.
...
> [!NOTE]
> User IDs are opaque strings. FlowWeft preserves case, does not trim whitespace, and rejects ISO control characters. Normalize user IDs in the host before they reach FlowWeft.
...
FlowWeft proceeds only when the complete security boundary exists. Missing context or ambiguous providers make the operation unavailable instead of silently broadening access.

| Situation | FlowWeft behavior |
|---|---|
| No `TenantProvider` bean | Operations requiring tenant context return `FEATURE_UNAVAILABLE` |
| Two `TenantProvider` beans without explicit resolution | Operation fails fast; FlowWeft does not guess |
...
> [!WARNING]
> A fallback that says "everyone can read if the provider is missing" is a silent privilege escalation. FlowWeft does not do that.
...
5. Only if all three succeed does FlowWeft execute the use case.
...
- [Plugins](../extensions/plugins.md) — extend FlowWeft with trusted in-process plugins.
