...
## What problem does FlowWeft solve?
...
FlowWeft is a Kotlin/JVM infrastructure framework that gives you a stable foundation for document lifecycles, storage abstraction, approvals, delivery, and diagnostics — without taking over your identity provider, folder tree, or business policies.

> [!TIP]
> Think of FlowWeft as the engine room, not the captain's bridge. It keeps the file machinery reliable; your host decides who can enter and what the files mean.

## What FlowWeft is not
...
## What FlowWeft owns vs. what your host owns

| FlowWeft owns | Your host owns |
| --- | --- |
...
FlowWeft is built around a few non-negotiable assumptions:

1. **External systems fail.** Storage buckets, downstream connectors, and AI services will timeout or return errors. FlowWeft commits local business state first, records durable work in the same transaction, and calls external systems outside long-running database transactions.
...
3. **Fail closed.** Missing tenant context or ambiguous provider resolution makes an operation unavailable — FlowWeft does not silently expand access.
4. **Doctor is a first-class feature.** Major components expose diagnostics so operators can see problems before users do.

> [!NOTE]
> FlowWeft targets JDK 8 as a baseline and is verified on JDK 21. Public APIs stay Java-friendly: no `suspend`, `Flow`, `value class`, `sealed interface`, or `data object` in SPI contracts.

## Choose your entry point

FlowWeft offers three ways to plug in. Pick the one that matches how much control you need:
...
> [!WARNING]
> Do not mix Boot 2 and Boot 3 starters in the same application. Choose one generation and align every FlowWeft artifact to it.

## Next steps

- [Install FlowWeft 0.0.3](installation.md)
