...
2. Add the remotely verified FlowWeft 0.0.3 coordinates to a Spring Boot 3 project.
...
Run a local PostgreSQL container with a database and user for FlowWeft:
...
## Step 2: Add FlowWeft dependencies
...
The `documentId` and `versionId` come from the committed aggregate. FlowWeft deliberately does not perform a second read after the command, so the response succeeds even if the caller does not hold read permission.
