...
> [!NOTE]
> FlowWeft does not expose storage URLs through the public protocol. Downloads return binary streams with `attachment`, `nosniff`, and `private, no-store` headers. Range, HEAD, ETag, and Content-Range are not supported.
