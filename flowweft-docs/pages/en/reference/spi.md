...
Enterprise deployments rarely share the same identity provider, object store, or review system. Hard-coding any of them would turn FlowWeft from infrastructure into a product. The SPI keeps the dependency arrow pointing one way:
...
Your host application implements the SPI, FlowWeft calls your beans, and your vendor SDKs stay inside your adapters.
...
FlowWeft never trusts a `tenantId` from request parameters. It asks the host for the current context.
...
A storage adapter only materializes bytes. FlowWeft owns object names, metadata, and tenant-scoped prefixes.
...
> [!TIP]
> Treat connectors as unreliable. Always implement timeout, retry, and idempotency; FlowWeft routes failures through the Outbox so operators can retry a single target.
...
FlowWeft discovers beans through Spring auto-wiring. Bean priority is:
