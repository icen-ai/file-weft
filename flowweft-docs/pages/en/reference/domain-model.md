...
Route resolution runs outside the FlowWeft database transaction so host policy can query external systems safely.

## Delivery target

When a document is published, FlowWeft creates a delivery attempt for each target in the active sync profile.
...
The domain emits Outbox events instead of calling external systems directly. For example, when a document is taken offline or archived, FlowWeft writes `document.delivery.target.removal.requested` events so each downstream target can be cleaned up asynchronously.
...
> [!WARNING]
> Do not trust `tenantId` from request parameters. FlowWeft always asks the `TenantProvider` for the current trusted tenant.
