...
| `RANGE_NOT_SUPPORTED` | Range requests are not supported for FlowWeft downloads. | Request the full resource. |
...
| `INTERNAL_ERROR` | An unexpected failure occurred inside FlowWeft. | Retry with backoff and contact operators if it persists. |
...
When you replay a command with the same `Idempotency-Key`, FlowWeft returns the original result if the fingerprint matches. If the key matches but the command differs, you receive `CONFLICT`:
