...
Enterprise files rarely move in a single request. A document may need review, a 2 GB video may need resumable upload, and OCR or scanning may need to process content asynchronously. FlowWeft keeps each concern explicit, persistent, and isolated.

## 1. Approval routing

The `DocumentReviewRouteProvider` SPI lets the host decide who must approve a document before it is published. Resolution happens outside the FlowWeft database transaction, so your HR or BPM system can be queried safely.
...
1. On `submit`, FlowWeft selects exactly one provider: the request's `reviewRouteId`, or the route configured by `fileweft.workflow.default-review-route-id` when the request omits it.
...
> [!CAUTION]
> `FileWeftTaskHandler` is the current generic durable-task capability; it is not FlowWeft Agent. The `fileweft-agent` artifact, Agent SPI/ABI and related migrations remain only for compatibility, and the 0.0.2 and 0.0.3 default product surfaces do not register or expose Agent.
...
6. After all approvals, FlowWeft publishes and triggers delivery connectors.
