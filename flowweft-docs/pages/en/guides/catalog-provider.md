...
FlowWeft deliberately does not store folder paths or folder ACLs. Instead, it asks the host for an opaque folder identifier through the `DocumentCatalogProvider` SPI and persists only that reference as asset metadata. This keeps your directory model authoritative while letting FlowWeft attach documents to the right place.

## 1. Why a catalog provider matters

Imagine FlowWeft as a warehouse that stores boxes. It knows what is inside each box, but it does not know the floor plan of your building. The catalog provider is the map: it tells FlowWeft which room a box belongs to, without FlowWeft needing to own the blueprint.
...
When a folder-aware host is configured, the create-document request requires a `folderId`. FlowWeft validates the folder through the provider before persisting the opaque reference.
...
If a user cannot see the requested folder, return `null` or omit it from the list. FlowWeft treats this as a validation failure instead of creating the document in an invisible folder.
...
FlowWeft keeps the opaque reference. It is the host's responsibility to decide whether deletion should block new uploads or orphan existing documents.

**Q: Does FlowWeft cache folder lists?**
No. FlowWeft calls the provider when validating create or update requests, so your host controls freshness and consistency.
