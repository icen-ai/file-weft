...
> [!WARNING]
> Do not read `X-Tenant-Id` or any other client header directly inside FlowWeft. Trust only the context established by your authentication layer.

## 2. Database query isolation

All FlowWeft business tables include `tenant_id`. Repositories filter every query by the current tenant context. Even aggregate or count queries are scoped.
...
| Caches | Cache keys must include the tenant ID. FlowWeft does not provide a shared cross-tenant cache. |
...
Yes. FlowWeft uses `tenant_id` row-level isolation. Whether you use one schema or many is a host operational choice.
