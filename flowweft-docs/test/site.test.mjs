...
const currentProductDecisionPages = [
  "pages/en/project/roadmap.md",
  "pages/zh/project/roadmap.md",
  "../docs/implementation-status.md",
];
...
test("site shell uses local assets and exposes accessible controls", async () => {
  ...
  assert.match(html, /<title>FlowWeft Documentation<\/title>/u);
  assert.match(html, />FLOWWEFT<small>DOCUMENTATION \/ 0\.0\.3<\/small>/u);
  assert.doesNotMatch(html, /FileWeft|FILEWEFT/u);
  assert.match(html, /DOCUMENTATION \/ 0\.0\.3/u);
  const packageJson = JSON.parse(await read("package.json"));
  assert.equal(packageJson.name, "flowweft-docs");
...
test("interaction script includes hash routing, search shortcuts and copy support", async () => {
  ...
  assert.match(source, /from "\.\/locale-storage\.js"/u);
  assert.match(source, /`\$\{meta\.nav\} — FlowWeft`/u);
...
test("legacy locale preference migrates once to the FlowWeft key", () => {
  const values = new Map([[legacyLocaleStorageKey, "zh"]]);
  const reads = [];
  const storage = {
    getItem(key) {
      reads.push(key);
  ...
  assert.equal(readLocale(storage), "zh");
  assert.deepEqual(reads, [localeStorageKey, legacyLocaleStorageKey]);
  assert.equal(values.get(localeStorageKey), "zh");
  assert.equal(values.has(legacyLocaleStorageKey), false);

  reads.length = 0;
  assert.equal(readLocale(storage), "zh");
  assert.deepEqual(reads, [localeStorageKey], "the migrated preference must not reread the legacy key");

  saveLocale("en", storage);
  assert.equal(values.get(localeStorageKey), "en");
});

test("FlowWeft brand reaches the local server and documentation package", async () => {
  const server = await read("server.mjs");
  const readme = await read("README.md");
  assert.match(server, /FlowWeft docs:/u);
  assert.doesNotMatch(server, /FileWeft docs:/u);
  assert.match(readme, /^# FlowWeft Docs$/mu);
});

test("visual system preserves FlowWeft tokens and responsive behavior", async () => {
  ...
  for (const page of hostIntegrationPages) {
    ...
    for (const block of dependencyBlocks) {
      assert.match(
        ...
        `${page} must keep the host JDBC starter beside the FlowWeft runtime starter`,
      );
    }
  }
});

test("every Boot 2 host guide pins FlowWeft's Kotlin runtime contract", async () => {
  for (const page of hostIntegrationPages) {
    const source = await read(page);
...
test("0.0.2 history stays immutable while the current roadmap delivers the redesigned Agent in 1.0", async () => {
  for (const page of releaseScopePages) {
    ...
    assert.match(
      source,
      /(?:0\.0\.2[^\n]{0,160}(?:does not provide|did not provide)|(?:does not provide|did not provide)[^\n]{0,160}0\.0\.2|0\.0\.2[^\n]{0,160}不提供|不提供[^\n]{0,160}0\.0\.2)/iu,
      `${page} must preserve that 0.0.2 did not provide Agent capability`,
    );
    assert.doesNotMatch(
      source,
  ...
  for (const page of currentProductDecisionPages) {
    const source = await read(page);
    assert.match(source, /1\.0(?:\.0)?/u, `${page} must name the 1.0 target`);
    assert.match(
      source,
      /(?:includes?[^\n]{0,120}(?:redesigned )?Agent|Agent[^\n]{0,120}(?:1\.0[^\n]{0,80}(?:deliver|include)|1\.0[^\n]{0,80}(?:交付|包含))|1\.0[^\n]{0,120}(?:Agent|智能))/iu,
      `${page} must make the redesigned Agent a 1.0 deliverable`,
    );
    assert.match(
      source,
      /(?:compatibility-only|compatibility|兼容)[\s\S]{0,220}(?:not be repurposed|不改义|不得改义|只作兼容|只为兼容)/iu,
      `${page} must keep the legacy Agent surface compatibility-only`,
    );
  }
});
