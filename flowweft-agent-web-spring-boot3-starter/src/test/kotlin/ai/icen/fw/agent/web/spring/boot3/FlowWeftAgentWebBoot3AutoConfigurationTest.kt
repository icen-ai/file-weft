...
class FlowWeftAgentWebBoot3AutoConfigurationTest {
    @Test
    fun `does not expose routes without trusted identity or when disabled`() {
        runner().withUserConfiguration(MapperOnlyConfiguration::class.java).run { context ->
            assertTrue(context.getBeansOfType(FlowWeftAgentWebBoot3Controller::class.java).isEmpty())
        }
        runner()
    ...
    fun `registers all adapter beans for one trusted provider`() {
        runner().withUserConfiguration(SecureHostConfiguration::class.java).run { context ->
            assertEquals(1, context.getBeansOfType(FlowWeftAgentWebBoot3ApplicationPorts::class.java).size)
            assertEquals(1, context.getBeansOfType(FlowWeftAgentWebBoot3JsonCodec::class.java).size)
            assertEquals(1, context.getBeansOfType(FlowWeftAgentWebBoot3Controller::class.java).size)
        }
    }
    ...
    fun `registers through Boot 3 auto configuration imports`() {
        val names = ClassPathResource(
            "META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports",
        ).inputStream.bufferedReader(Charsets.UTF_8).use { it.readLines() }
        assertTrue(names.contains(FlowWeftAgentWebBoot3AutoConfiguration::class.java.name))
    }

    private fun runner(): WebApplicationContextRunner = WebApplicationContextRunner()
        .withConfiguration(AutoConfigurations.of(FlowWeftAgentWebBoot3AutoConfiguration::class.java))
