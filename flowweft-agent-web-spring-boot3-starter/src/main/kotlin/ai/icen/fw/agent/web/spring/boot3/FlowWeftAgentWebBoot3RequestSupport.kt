...
class FlowWeftAgentWebBoot3JsonCodec(objectMapper: ObjectMapper) {
...
class FlowWeftAgentWebBoot3Response<T>(
    val code: String,
    val data: T?,
...
internal class FlowWeftAgentWebBoot3RequestSupport(
    private val codec: FlowWeftAgentWebBoot3JsonCodec,
    ...
    private fun <T : Any> project(
        ...
        version: (T) -> Long?,
    ): ResponseEntity<FlowWeftAgentWebBoot3Response<T>> {
        ...
        return ResponseEntity.status(AgentWebHttpStatusPolicy.statusFor(result.code))
            .headers(headers)
            .body(FlowWeftAgentWebBoot3Response(result.code.value, result.value, result.replayed))
    }
    ...
    private fun failure(code: AgentWebErrorCode): ResponseEntity<FlowWeftAgentWebBoot3Response<Any>> =
