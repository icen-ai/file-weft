...
/** Package-private JDK 8 validation, immutability, and canonical digest support. */
final class InteroperabilityContractSupport {
    ...
    static final int MAX_MEDIA_TYPE_UTF8_BYTES = 256;
    static final int MAX_CAPABILITIES = 256;
    static final int MAX_DESCRIPTORS = 512;
    ...
    private InteroperabilityContractSupport() {
    }
    ...
    static String requireMediaType(String value, String message) {
        requireText(value, MAX_MEDIA_TYPE_UTF8_BYTES, message);
        require(value.indexOf('/') > 0 && value.indexOf('/') == value.lastIndexOf('/'), message);
        for (int index = 0; index < value.length(); index++) {
            char character = value.charAt(index);
            require(character > 0x20 && character < 0x7F &&
    ...
    static final class DigestWriter {
        ...
        private DigestWriter(String domain) {
            requireMachineCode(domain, "Interoperability digest domain is invalid.");
            require(domain.startsWith("flowweft.agent.interoperability."),
                "Interoperability digest domain is invalid.");
            this.digest = newSha256();
            text(domain);
        }

        DigestWriter text(String value) {
            require(!finished, "Interoperability digest writer is finalized.");
            Objects.requireNonNull(value, "Interoperability digest text must not be null.");
