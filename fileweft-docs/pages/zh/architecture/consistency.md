...
3. **Worker 带租约领取**：异步 Worker 使用时间受限的租约捡取 Outbox 记录，避免多节点竞争。
...
> **TIP**
> 连接器实现应把 `ConnectorSyncRequest` 当作幂等键。即使上一次 HTTP 响应丢失，重放同一请求也必须产生相同的逻辑结果。
...
> **WARNING**
> 不要仅按时间年龄编写清理任务来删除对象。必须先与 PostgreSQL 引用对账，否则会误删提交记录延迟的真实数据。
...
> **NOTE**
> FileWeft 不会回滚已经成功的连接器。交付是追加式的：成功的目标保持发布，失败的目标被重试或上报。
