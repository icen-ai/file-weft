...
当前发布合同是 `ai.icen:*:0.0.2`。只有受保护 `v0.0.2` 标签流水线成功，且匿名冷缓存消费者能够解析精确远端制品时才应消费；否则继续使用已经完成同等证据的最近版本。

### 还能使用 `com.fileweft:*` 制品吗？

不能。这些试推制品已撤回。请改用已完成远端验证的 `ai.icen:*:0.0.2` 坐标。

### 如何确认 0.0.2 可以消费？

必须同时确认精确提交的全部匹配 lane、受保护标签发布，以及从空缓存匿名解析远端制品。只有源码或标签名称还不够。
