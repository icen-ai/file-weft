...
FileWeft 是面向企业的 Kotlin/JVM 文件智能基础设施框架，覆盖文档生命周期、审批、多目标交付、存储抽象、诊断与审计。它不是简单的上传模块、业务文档系统、Dify/ESE 包装或云存储 SDK。
...
不会。FileWeft 从你的 `TenantProvider` 消费租户上下文，并将其应用到查询、存储路径、事件、任务、日志和缓存。
...
`ai.icen:*:0.0.1`。`0.0.2-SNAPSHOT` 正在开发中，不应视为稳定版。

### 还能使用 `com.fileweft:*` 制品吗？

不能。这些试推制品已撤回。请改用 `ai.icen:*:0.0.1`。

### 0.0.2 什么时候发布？

只有当 [路线图](project/roadmap) 中所有验收项都能从干净环境复现，且远端制品已验收后，才会发布。
