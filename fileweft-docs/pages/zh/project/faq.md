...
FileWeft 是面向企业的 Kotlin/JVM 文件智能基础设施框架，覆盖文档生命周期、审批、多目标交付、存储抽象、诊断与审计。它不是简单的上传模块、业务文档系统、Dify/ESE 包装器或云存储 SDK。

### FileWeft 是一个开箱即用的文件上传组件吗？

不是。FileWeft 提供基础能力；宿主需要通过 SPI 实现提供身份、租户、授权、目录树和业务规则。
...
不会。FileWeft 从 `TenantProvider` 获取租户上下文，并将其应用到查询、存储路径、事件、任务、日志和缓存。
...
> FileWeft 从不信任调用方发送的 `tenantId`，始终通过配置的 `TenantProvider` 解析当前租户。

## 存储

### FileWeft 会替代 S3 或 MinIO 吗？

不会。FileWeft 通过 `StorageAdapter` 抽象存储，让你可以继续使用 S3、MinIO、OSS 或自定义后端，而不把厂商细节泄漏到领域层。

### 可以直接从对象存储提供文件访问吗？

FileWeft 不在公共 HTTP API 中暴露对象存储预签名 URL。授权下载由应用以二进制流返回，响应头固定为 `attachment`、`nosniff` 和 `private, no-store`。

## 发布

### 当前发布合同是什么？

当前发布合同是 `ai.icen:*:0.0.3`。只有受发布门禁约束的 `v0.0.3` 标签匹配受保护远端 `main` HEAD、精确提交的全部必需门禁成功，且匿名冷缓存消费者能够回读全部 19 个坐标及 Boot 2、Boot 3、纯 SPI 消费者时才应消费；否则继续使用已经完成同等证据的最近版本。
...
必须同时确认受发布门禁约束的 `v0.0.3` 标签匹配受保护远端 `main` HEAD、精确提交的全部匹配 lane，以及用全新隔离缓存匿名回读全部 19 个坐标和三种消费者。只有源码、本文、标签名称或部分绿灯都不够。
