...
4. **存储链路** —— 共享持久化 `StorageAdapter` 契约，含本地文件系统与 S3 兼容适配器。
...
使用 `ai.icen` group。旧版 `com.fileweft:*` 试推制品已撤回，不会自动收养。
...
- 稳定版是 `0.0.1`。任何标记为 `0.0.2-SNAPSHOT` 的都是尚未发布的开发线。
...
不能。这些试推制品已撤回。请迁移到 `ai.icen:*:0.0.1`。
...
- [安装 0.0.1](getting-started/installation)。
