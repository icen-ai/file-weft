...
FileWeft 需要一个名为 `fileweftDataSource` 的 `javax.sql.DataSource` Bean（如果只有一个 DataSource，则使用主 Bean）。由你创建、调优连接池并负责凭据轮换。
...
你也可以列出插件，确认存储适配器、目录 Provider 或连接器 Bean 已注册：
