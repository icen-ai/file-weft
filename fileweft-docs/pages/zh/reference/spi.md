...
企业部署很少共享同一个身份提供商、对象存储或审批系统。如果把这些硬编码进来，FileWeft 就会从基础设施退化成产品。SPI 保证依赖方向始终单向：
...
宿主应用实现 SPI，FileWeft 调用你的 Bean，而厂商 SDK 只留在你的适配器里。
...
FileWeft 不会信任请求参数里的 `tenantId`，它会向宿主索取当前上下文。
...
存储适配器只负责落地字节。对象名、元数据和租户前缀由 FileWeft 负责。
...
> [!TIP]
> 把连接器当作不可靠的外部系统。务必实现超时、重试和幂等；FileWeft 会把失败路由到 Outbox，让运维人员可以针对单个目标重试。
...
FileWeft 通过 Spring 自动装配发现 Bean，优先级为：
