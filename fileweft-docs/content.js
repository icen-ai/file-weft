...
export const pages = {
  ...
  "project/roadmap": page("project", 5,
    {
      ...
      sections: [
        { title: "How to read the roadmap", html: `${table(["State", "Meaning"], [["Planned", "Scope is agreed, but no completion claim is allowed."], ["In progress", "Some implementation or evidence exists; every unchecked acceptance item remains release-blocking."], ["Complete", "All acceptance evidence is reproducible from a clean environment and the completion boundary is satisfied."]])}${note("!", "Evidence is version-specific", "Do not copy a success from another database, Boot generation, vendor emulator or earlier release into a completion claim. If evidence cannot be reproduced, the item remains open.", true)}` },
        { title: "0.0.2 · close the current development line", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["Runtime-closure SBOM; SNAPSHOT release verifier; formal resumable-upload, catalog and agent HTTP resources.", "The SBOM verifier derives every published module's actual runtime closure and rejects test, compiler and build-tool leakage; the release verifier proves SNAPSHOT/stable version rules; Boot 2 and Boot 3 contract, context and MVC tests plus browser E2E prove tenant, permission, cursor, error and redaction behavior for all three HTTP families.", "Do not publish 0.0.2 while the SBOM is only a dependency dump, while a verifier can accept the wrong release identity, or while any resource exists only as an internal service, dev-only route or one-Boot-generation controller."]])}${note("IN PROGRESS", "Current hand-off status", "Runtime-closure SBOM and metadata-driven timestamped SNAPSHOT repository verification are implemented and locally verified. Still open: release/SNAPSHOT fixture tests and corrupt, duplicate, XXE, path-traversal and mixed-build negatives; exact repository inventory, artifact-level metadata/checksum and dangerous JAR-entry checks; formal resumable-upload, catalog and agent HTTP resources with dual-Boot tests and browser E2E; then the final clean release gate.")}` },
        { title: "0.1.0 · production supply chain", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["CNB CI using confirmed official syntax; OSV vulnerability policy; artifact signing and provenance; remote cold-cache consumption; one release identity from source to registry.", "A clean CNB runner executes the full gate with the syntax verified against current CNB documentation; a pinned OSV scan enforces an explicit severity/exception policy; consumers verify signatures and provenance; a clean machine with empty Gradle caches resolves and runs the published artifacts from CNB; tag, commit, POM, module metadata, SBOM and provenance all report the same version and source revision.", "Local publication, a warm developer cache, an unsigned checksum, an unverified sample YAML or a registry page screenshot cannot be used to claim a production release pipeline."]])}` },
        { title: "0.2.0 · databases and host integration", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["MySQL 8 support; wider reusable TestKit coverage for host SPIs; structured logging and OpenTelemetry integration.", "Fresh-install, supported-version upgrade, concurrency, tenant-isolation and rollback-safe migration suites pass against real MySQL 8 as well as PostgreSQL; external sample hosts run identity, authorization, tenant, catalog, workflow, storage, connector and agent contract kits; an OpenTelemetry Collector test asserts trace/metric/log correlation, redaction and bounded cardinality.", "H2 compatibility, SQL parsing, mocks, a single happy-path adapter test or visually inspected logs do not prove MySQL, SPI or observability support."]])}` },
        { title: "0.3.0 · operations at scale", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["Retention and legal-hold rules; partition lifecycle; capacity envelopes; declared RPO/RTO; backup and restore procedures; measurable SLOs.", "Time-controlled retention and hold tests prove safe deletion; partition create/rotate/archive/drop tests preserve tenant and audit invariants; repeatable load reports publish tested document, version, object and queue limits; fault drills restore database and object storage into a clean environment within the declared RPO/RTO; dashboards and alerts are exercised against agreed availability, latency and backlog SLOs.", "Configuration examples, an untested runbook, synthetic throughput without bottleneck disclosure, or a backup that has never been restored cannot be described as production-scale readiness."]])}` },
        { title: "0.4.0 · official vendor adapters", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["Maintained adapters for OSS, Dify, ESE and AppBuilder against their official services.", "Credential-isolated contract environments prove the documented vendor API versions, authentication, timeout, retry classification, idempotent delivery, safe repeated removal, pagination or upload limits, redacted failures, health checks and recovery; compatibility and ownership are documented per adapter.", "A generic S3 implementation, WireMock response, reverse-engineered payload or dev simulator does not establish official vendor support. An adapter without repeatable real-service evidence remains experimental and must not be advertised as supported."]])}` },
        { title: "1.0.0 · stable compatibility line", html: `${table(["Deliverables", "Acceptance evidence", "Completion boundary"], [["Public API and ABI freeze; supported migration and upgrade matrix; security audit; complete operator, integrator and compatibility documentation.", "Binary and source compatibility gates compare every public artifact with the declared baseline; clean installs and every supported version-to-version upgrade pass across the published JDK, Spring Boot and database matrix; threat models, dependency findings, tenant/authorization/storage abuse cases and remediation are reviewed; every documented command and example is executable; the support and deprecation policy names exact versions and dates.", "Do not label the project 1.0 while a public contract is still provisional, a supported upgrade path is untested, a high-severity security finding is unresolved, or compatibility claims exceed the verified matrix."]])}${note("1.0", "Stability is a maintained obligation", "Passing the 1.0 gate freezes the declared contracts; later removals require the published deprecation window, migration guidance and compatibility evidence. A tag alone does not create stability.")}` },
      ],
    },
    {
      title: "用证据推进的开发路线图",
      ...
      sections: [
        { title: "如何阅读路线图", html: `${table(["状态", "含义"], [["计划中", "范围已确定，但禁止宣称已经完成。"], ["进行中", "已有部分实现或证据；任何未验收项都继续阻塞发布。"], ["已完成", "所有验收证据都能从干净环境复现，并满足完成边界。​"]])}${note("!", "证据只对被验证版本有效", "不能把另一种数据库、另一个 Boot 代际、厂商模拟器或旧版本的成功结果挪用为本版本的完成声明。证据不能复现时，该项仍是待办。", true)}` },
        { title: "0.0.2 · 收口当前开发线", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["基于运行时闭包的 SBOM；SNAPSHOT 发布校验器；正式的断点续传、目录与 Agent HTTP 资源。", "SBOM 校验器从所有发布模块的真实 runtime closure 推导组件，并拒绝测试、编译器和构建工具泄漏；发布校验器证明 SNAPSHOT/正式版本规则；Boot 2 与 Boot 3 的契约、Context、MVC 测试及浏览器 E2E 覆盖三组 HTTP 的租户、权限、游标、错误和脱敏行为。", "SBOM 仍只是依赖转储、校验器会接受错误发布身份，或任一资源仅存在于内部服务、dev 路由或单一 Boot 代际时，都不能发布 0.0.2。​"]])}${note("进行中", "当前交接状态", "运行时闭包 SBOM 与基于 Maven metadata 的时间戳 SNAPSHOT 仓库校验已经实现并通过本地验证。仍待完成：正式版/SNAPSHOT fixture 及损坏、重复、XXE、路径穿越、混合构建负例；仓库精确库存、artifact 级 metadata/checksum 与危险 JAR entry 校验；正式断点续传、目录、Agent HTTP 资源及其双 Boot 测试、浏览器 E2E；最后再跑干净发布门禁。")}` },
        { title: "0.1.0 · 生产供应链", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["使用已确认官方语法的 CNB CI；OSV 漏洞策略；制品签名与 provenance；远端冷缓存消费；从源码到仓库唯一发布身份。", "干净 CNB Runner 使用当前 CNB 官方文档确认过的语法跑完全门禁；固定版本的 OSV 扫描执行明确的严重度和例外策略；消费者可验证签名与 provenance；空 Gradle 缓存的新机器只从 CNB 解析并运行正式制品；Tag、Commit、POM、Module Metadata、SBOM 与 provenance 的版本和源码修订完全一致。", "本地 publish、开发者热缓存、未签名 checksum、未经官方确认的示例 YAML 或仓库页面截图，都不能用于宣称生产发布链路完成。​"]])}` },
        { title: "0.2.0 · 数据库与宿主集成", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["MySQL 8 支持；扩大可复用 TestKit 的宿主 SPI 覆盖；结构化日志与 OpenTelemetry 集成。", "真实 MySQL 8 和 PostgreSQL 都通过全新安装、受支持版本升级、并发、租户隔离及可安全回滚的迁移套件；外部样例宿主运行身份、授权、租户、目录、工作流、存储、连接器与 Agent 契约套件；OpenTelemetry Collector 测试断言 Trace/Metric/Log 关联、脱敏和有界基数。", "H2 兼容、SQL 解析、Mock、单一 happy path 适配器测试或肉眼查看日志，都不能证明 MySQL、SPI 或可观测性支持。​"]])}` },
        { title: "0.3.0 · 规模化运营", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["保留与 legal hold 规则；分区生命周期；容量边界；明确 RPO/RTO；备份恢复流程；可度量 SLO。", "可控时间测试证明保留、冻结与安全删除；分区创建、轮转、归档和删除测试保持租户及审计不变量；可重复压测报告公开经过验证的文档、版本、对象和队列上限；故障演练将数据库与对象存储恢复到干净环境，并达到声明的 RPO/RTO；仪表盘和告警按约定可用性、延迟与积压 SLO 实际触发。", "配置示例、未演练 Runbook、未披露瓶颈的合成吞吐量，或从未恢复过的备份，都不能描述为生产规模就绪。​"]])}` },
        { title: "0.4.0 · 官方厂商适配器", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["面向 OSS、Dify、ESE 与 AppBuilder 官方服务的持续维护适配器。", "隔离凭据的合约环境验证文档所列厂商 API 版本、认证、超时、重试分类、幂等交付、安全重复撤回、分页或上传限制、错误脱敏、健康检查和故障恢复；每个适配器都记录兼容范围与维护责任。", "通用 S3 实现、WireMock 响应、逆向 payload 或 dev 模拟器不能证明官方厂商支持。没有可重复真实服务证据的适配器必须保持 experimental，不能宣传为受支持。​"]])}` },
        { title: "1.0.0 · 稳定兼容线", html: `${table(["交付内容", "验收证据", "禁止冒充完成的边界"], [["公共 API/ABI 冻结；受支持迁移升级矩阵；安全审计；完整运维、集成与兼容文档。", "二进制及源码兼容门禁将每个公共制品与声明基线比较；全新安装和所有受支持版本间升级覆盖已公布的 JDK、Spring Boot 与数据库矩阵；威胁模型、依赖发现、租户/授权/存储滥用场景和修复经过评审；文档中的命令与示例全部可执行；支持和弃用策略列出精确版本与日期。", "公共契约仍为 provisional、受支持升级路径未测试、高危安全问题未解决，或兼容声明超出实际验证矩阵时，禁止标记为 1.0。​"]])}${note("1.0", "稳定性是持续义务", "通过 1.0 门禁即冻结已声明契约；后续移除必须遵守公开弃用窗口，提供迁移指导和兼容证据。仅创建 Tag 不会自动产生稳定性。")}` },
      ],
    }),
};
